<template>
  <div>
    ...
    <div class="button-logout"  @click="unbind">
     解除绑定
    </div>
...
export default {
  ...
  methods: {
    ...
    unbind() {
       ...
       this.util.delCookieByName('IDSTGC');
       let url = window.location.href.split("#")[0];
      if (!window.base.IsLocalLogin){
          url = window.base.loginToUrl;
          this.util
            .postAjax({
              code: this.code.authbase,
              url: this.code.weixinUnbind,
              data: {
    ...
    gomonitor(){
      this.$router.push("monitorHistory");
    },
    gobreakrecord() {
