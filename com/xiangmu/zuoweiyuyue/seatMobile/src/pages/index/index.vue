<template>
  <div>
    ...
    <!-- <record></record> -->
    <div class="index_content" v-show="!isqrcode">
      <div class="index_tab">
        <ul>
      ...
      <div class="sign_box" v-if="signList">
        ...
        <div>
          <i class="time"></i>
          今日 {{ signList.signStarttime.substring(8, 10) }}:{{
            signList.signStarttime.substring(10, 12)
          }}:{{ signList.signStarttime.substring(12, 14) }}～{{
            signList.signEndtime.substring(8, 10)
          }}:{{ signList.signEndtime.substring(10, 12) }}:{{
            signList.signEndtime.substring(12, 14)
          }}
        </div>
      ...
      <div class="using" v-if="usingDetail">
        ...
        <div class="using_detail">
          ...
          <p>
            {{ usingDetail.starttime.substring(0, 2) }}:{{
              usingDetail.starttime.substring(2, 4)
            }}～{{ usingDetail.endtime.substring(0, 2) }}:{{
              usingDetail.endtime.substring(2, 4)
            }}
          </p>
        </div>
        <div class="line"></div>
        ...
        <div
          class="using_bottom using_bottom1 clearfix"
          v-if="usingDetail.status == 2"
        >
          <div @click="cancelSeat">退 座</div>
        </div>
      </div>
      <div class="now_appoint" v-if="currentOrders.length > 0">
        ...
        <div class="clearfix" v-for="(v, i) in currentOrders" :key="i">
          <div class="appoint_left" @click="seeTimedetail(v)">
            ...
            <p v-if="v.dayfilter.length > 1">
              {{ v.dayfilter[0].substring(0, 4) }}-{{
                v.dayfilter[0].substring(4, 6)
              }}-{{ v.dayfilter[0].substring(6, 8) }}~{{
                v.dayfilter[v.dayfilter.length - 1].substring(0, 4)
              }}-{{ v.dayfilter[v.dayfilter.length - 1].substring(4, 6) }}-{{
                v.dayfilter[v.dayfilter.length - 1].substring(6, 8)
              }}
            </p>
            <p v-else>
              {{ v.dayfilter[0].substring(0, 4) }}-{{
      ...
      <div class="now_emptyseat">
        ...
        <div>
          ...
          <van-list
            class="occupy_seat"
            v-if="showList || occupyList.length"
          ...
          >
            <div>
              <div
                class="single_occupy_seat"
                v-for="(v, i) in occupyList"
                :key="i"
              >
                ...
                <h4 v-if="v.opentime" :class="{ fulltext: v.unusenum == 0 }">
                  当前剩余：{{ v.unusenum }}座
                </h4>
                <i
                  @click="goOccupy(v)"
    ...
    <van-overlay :show="showLeave">
      <div class="Leavewrapper" @click.stop>
        <div>
          <div class="img"></div>
          <h3>选择暂离类型：</h3>
          <p @click="diningLeave('2')" :class="{ noleave: !candineLeave }">
            就餐暂离
          </p>
          <p @click="diningLeave('1')" :class="{ noleave: !cantemporaryLeave }">
            临时暂离
          </p>
...
export default {
  ...
  methods: {
    ...
    sign(id) {
      if (window.base.isSeatYyzc) {
        let scan = window.sendMsgToApp("appointment");
        if (scan && scan.then) {
          scan.then(res => {
            let url = res;
            let token = this.util.getUrlParam("token");
            let key = this.util.getUrlParam("key");
            this.util
              .postAjax({
                code: "idsweb",
                url: "/seatYyzc/getUser",
                data: {
                  key,
                  token
                },
      ...
      if (this.scanType === "wx_uniapp_mp") {
        ...
        var xcurl = window.base.isXj
          ? window.base.uniapp_mp_scan +
            "?callBackUrl=" +
            encodeURIComponent(window.base.scan_callback)
          : window.base.uniapp_mp_scan;
  ...
  mounted() {
    if (!window.base.hwwxindex || this.util.getCookieByName("IDSTGC"))
      // 获取用户信息
    ...
    // });
    if (!this.util.getCookieByName("IDSTGC") && window.base.hwwxindex) {
      ...
      if (
        ...
        let scanType = window.base.WXSIGN_TYPE;
        // 微信客户端
        if (scanType === "wx") {
          this.initwx();
          this.scanType = "wx_normal";
        } else if (scanType === "uniapp_mp") {
