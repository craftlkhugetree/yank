...
global.url["seat_v2"] = window.base.baseURL;
global.url["authweb"] = window.base.authURL;
global.url["idsweb"] = window.base.idsURL;

// var IDSTGC = "0de747f29fa443b9a8677a33723baf2e";
var IDSTGC = "";
...
let startAjax = function(options) {
  return new Promise(function(resolve, reject) {
    if (options && options.url && options.code) {
      getRealUrl(options.code, options.url)
        .then(function(data) {
          let ajaxSettings = {
            url: data,
            method: "POST",
            headers: {
              IDSTGC: getCookie("IDSTGC") || IDSTGC
            },
            data: options.isRep
              ? options.data
          ...
          return Axios(ajaxSettings)
            .then(function(data) {
              let orgindata = data;
              let res = data.data;
              if (
                res.login === false ||
                (typeof res === "string" && res.includes("<html>"))
              ) {
                redirectToLogin();
              } else {
...
let getUrlParamByName = function(name) {
...
let redirectToLogin = function() {
  if (window.base.isSjzkj) {
    //alert(location.href);
    startAjax({
      code: "idsweb",
      url: "/Oauth2Sjzkg/getUser",
      data: {
        accessToken: getUrlParamByName("access_token")
      },
      isRep: false
    })
      .then(res => {
      ...
      .catch(err => {});
  } else if (window.base.isXj) {
    startAjax({
      code: "idsweb",
      url: "/seatXJ/getUser",
      data: {
        code: getUrlParamByName("code")
      },
      isRep: false
    })
      .then(res => {
      ...
      .catch(err => {});
  } else if (window.base.isSeatYyzc) {
    startAjax({
      code: "idsweb",
      url: "/seatYyzc/getUser",
      data: {
        key: getUrlParamByName("key"),
        token: getUrlParamByName("token")
      },
      isRep: false
    })
      .then(res => {
