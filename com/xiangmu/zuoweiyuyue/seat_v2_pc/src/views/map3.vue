  ...
  methods: {
    ...
    click() {
      ...
      navigator.geolocation.getCurrentPosition(
        function (position) {
          let box = '';
          box += '经度' + position.coords.longitude;
          box += '纬度' + position.coords.latitude;
          box += '准确度' + position.coords.accuracy;
          box += '海拔' + position.coords.altitude;
          box += '海拔准确度' + position.coords.altitudeAcuracy;
          box += '行进方向' + position.coords.heading;
          box += '地面速度' + position.coords.speed;
          box += '请求的时间' + new Date(position.timestamp);
          alert(box);
        },
        function (err) {
          console.log(err);
