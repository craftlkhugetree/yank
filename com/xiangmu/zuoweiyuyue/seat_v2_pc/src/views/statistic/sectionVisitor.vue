      ...
      <div class="base-search-table">
        ...
        <div class="left-div" v-if="tableData.length">
          <el-table
            ...
            empty-text=" "
          >
            <el-table-column prop="orderdate" label="日期" v-if="activeTab=='day'">
              <template
                slot-scope="scope"
              >{{ moment(scope.row.orderdate, "YYYYMMDDhhmmss").format("YYYY-MM-DD" ) }}</template>
            </el-table-column>
            <el-table-column prop="monthdate" label="月份" v-if="activeTab=='month'">
              <template slot-scope="scope">{{ scope.row.monthdate}}</template>
            </el-table-column>
            <el-table-column prop="yeardate" label="年份" v-if="activeTab=='year'">
              <template slot-scope="scope">{{ scope.row.yeardate+"年" }}</template>
            </el-table-column>
            <el-table-column prop="username" label="入馆人数">
        ...
        <div class="right-div" v-show="tableData.length ">
          <h4 class="h4-title">区间入馆人次曲线图</h4>
          <div id="sectionVisitor" style="height:650px;width:100%;"></div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    this.getSections();
    this.chart = echarts.init(document.getElementById("sectionVisitor"));
  },
  methods: {
    //获取全部的区间
    getSections() {
      fetchSections({}).then(res => {
        if (res.success) {
          ...
          this.section = res.data.length ? res.data[0].id : null;
          this.getTableData();
        }
      });
    },
