<template>
  <div class="base-search-table">
      ...
      <el-table-column prop="stepMove" label="排序" width="150">
        <template slot-scope="scope">
          <div class="mod16 flex-row justify-between">
            <span
              :class="beAbleToMove(scope.$index, UP) ? 'disable-span' : ''"
              @click="beAbleToMove(scope.$index, UP) ? void 0 : moveOneLine(scope.row.id, UP)"
            >
              ↑&nbsp;上移
            </span>
            <span
              :class="beAbleToMove(scope.$index, DOWN) ? 'disable-span' : ''"
              @click="beAbleToMove(scope.$index, DOWN) ? void 0 : moveOneLine(scope.row.id, DOWN)"
            >
              ↓&nbsp;下移
            </span>
          </div>
      ...
      <el-table-column label="操作" width="440" fixed="right">
        <template slot-scope="scope">
          ...
          <el-divider
            ...
            v-show="
              scope.row.status !== '1' ||
              (scope.row.isinterval == '0' && scope.row.status < '2') ||
              (scope.row.isinterval == '1' && scope.row.status !== '3')
            "
          ></el-divider>
          <span @click="copySign(scope.row)">签到地址</span>
        </template>
      </el-table-column>
    </el-table>
    <!---------------------------- 分页 ---------------------------->
    ...
    <close-dialog
      class="fixed-dialog"
      ref="closeDialog"
      @doFunc="getTableData(1, pageSize)"
    ></close-dialog>
    <!---------------------------- 查看座位弹窗 ---------------------------->
...
export default {
  ...
  computed: {
    beAbleToMove() {
      return (index, str) =>
        str === this.DOWN
          ? this.pageSize * (this.currentPage - 1) + index + 1 === this.total
  ...
  created() {
    // let v = 'IDSTGC';
    // let ids = this.util.getCookieByName(v);
    // console.log(ids);
    // this.util.delCookieByName(v);
    // ids = this.util.getCookieByName(v);
    // console.log('GMT del', ids);
    // if (ids) {
    //   var expires = new Date();
    //   expires.setTime(expires.getTime() - 10000);
    //   document.cookie =
    //     'IDSTGC=' + escape('echo') + ';expires=' + new Date(0).toUTCString() + `;Path=/`;
    //   console.log('UTC del', this.util.getCookieByName(v));
    // }
    this.getTableData(this.currentPage, this.pageSize);
  },
};
