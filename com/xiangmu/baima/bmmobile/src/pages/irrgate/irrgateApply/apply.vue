...
export default {
  ...
  computed: {
    ...
    ruleDayTime() {
      if (this.mostDay && this.mostDay != 0) {
        return this.ruleDay
          ? this.ruleTime
            ? `请最少提前${this.ruleDay}天${this.ruleTime}，最多提前${this.mostDay}天申请`
            : `请最少提前${this.ruleDay}天，最多提前${this.mostDay}天申请`
          : `请于${this.ruleTime}前申请`;
      } else {
        return this.ruleDay
          ? this.ruleTime
            ? `请最少提前${this.ruleDay}天${this.ruleTime}前申请`
            : `请最少提前${this.ruleDay}天申请`
          : `请于${this.ruleTime}前申请`;
      }
    },
    // 最小可选时间
    ...
    irMaxDate() {
      let days = parseInt(this.mostDay) || 0;
      let nowTime = this.util.formatTime(new Date().getTime(), 'hh:mm');
      if (this.ruleTime && this.ruleTime < nowTime) {
        days += 1;
      }
        ...
        .then(res => {
          if (res.success) {
            ...
            if (this.operType === 'applyEdit') {
              let obj =
                this.timeOption.find(t => this.applyForm.daytype == t.value) || this.timeOption[0];
              this.selectTime = obj.text;
  ...
  created() {
    if (!this.id) {
      ...
      this.applyForm.applyerid = userInfo.ID;
    } else {
      // 获取详情
      this.getDetail();
    }
