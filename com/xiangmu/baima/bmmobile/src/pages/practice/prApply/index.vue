  ...
  <div>
    ...
    <div class="tabs" ref="tableTabs">
      <li
        ...
        v-for="item in tabs"
        :key="item.value"
        @click="activeTableTab = item.value;"
      >{{item.text}}</li>
    </div>
  ...
  methods: {
    ...
    getMoreData() {
      this.getList(this.currentPage + 1).then(list => {
        ...
        this.finishTable = this.total === this.tableData.length
      });
    },
    // 获取列表
    getList(page) {
        ...
        this.util
          ...
          .then(res => {
            ...
            if (res.success == true) {
              ...
              this.total = applyerList.total || 0
