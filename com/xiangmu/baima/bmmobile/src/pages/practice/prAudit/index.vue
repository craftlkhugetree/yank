...
export default {
  ...
  computed: {
    curRole() {
      if(this.$route.path === '/practice-audit-hq') {
        sessionStorage.setItem('curRole', roleId.hq);
        return roleId.hq
      }
     if(this.$route.path === '/practice-audit-bm') {
        sessionStorage.setItem('curRole', roleId.bm);
        return roleId.bm
      } 
    },
    isRearService() {
  ...
  methods: {
    downloadForm(row) {
      ...
      let isSameDay = this.resDDetail.eatstarttime === this.resDDetail.eatendtime;
      let eatObj = {
        '1,1': '含早餐',
        '2,2': '含中餐',
        '3,3': '含晚餐',
      ...
      if (isSameDay) {
        let str =
          eatstarttype <= eatendtype
            ? eatstarttype + ',' + eatendtype
            : eatendtype + ',' + eatstarttype;
        this.resDDetail.eatstarttype = this.resDDetail.eatendtype = eatObj[str];
      } else {
        if (eatstarttype) {
          switch (eatstarttype) {
            case '1':
    ...
    changeTab(item) {
      if(this.activeTableTab === item.value) return
        ...
        this.util
          ...
          .then(res => {
            ...
            if (res.success == true) {
              ...
              this.total = this.operDev === 'leader' ? leaderList.total : bmList.total;
              if (page === 1) {
                this.tableData = list;
              }
