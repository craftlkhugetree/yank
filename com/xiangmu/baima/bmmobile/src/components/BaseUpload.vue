...
export default {
    ...
    getFile(e) {
      ...
      for (let i = 0; i < this.fileList.length; i++) {
        ...
        if (exts.indexOf(fileext) === -1) {
          this.$toast.fail('文件必须为：' + this.exts);
          throw '上传了不符合后缀的文件！';
        }
        if (this.size) {
          let limitFileSize = this.size * 1024;
          if (item.size > limitFileSize) {
            this.$toast.fail('文件必须小于：' + this.size);
            throw '上传文件超过限制大小！';
          }
        }
      }
      ...
      for (let i = 0; i < this.fileList.length; i++) {
        ...
        // formData.append("PHYAPPID", "15000-1");
      }

      let axiosSettings = {
        url: this.url,
        method: 'POST',
        headers: { IDSTGC: this.getCookie('IDSTGC') || '59386188698f47d5a7c4b16c6278d14b'},
        data: formData,
      };
      // 发送http请求
      axios(axiosSettings).then(data => {
        let res = data.data;
        if (res && Object.prototype.hasOwnProperty.call(res, 'success') && !res.success) {
  ...
  props: {
    ...
    isCarmera: Boolean !== false,
  },
  created() {
    this.exts = this.exts ? this.exts : 'jpg|png|bmp|gif|txt';
    this.carmera = this.isCarmera ? 'image/*' : '';
  },
};
</script>
