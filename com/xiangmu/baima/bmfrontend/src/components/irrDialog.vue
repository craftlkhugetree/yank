...
export default {
  ...
  computed: {
    // 规则信息
    limitDayTime() {
      if (this.mostDay && this.mostDay != 0) {
        return this.limitDay
          ? this.limitTime
            ? `请最少提前${this.limitDay}天，${this.limitTime}，最多提前${this.mostDay}天申请`
            : `请最少提前${this.limitDay}天，最多提前${this.mostDay}天申请`
          : `请于${this.limitTime}前申请`;
      } else {
        return this.limitDay
          ? this.limitTime
            ? `请最少提前${this.limitDay}天${this.limitTime}前申请`
            : `请最少提前${this.limitDay}天申请`
          : `请于${this.limitTime}前申请`;
      }
    }
  },
  methods: {
    ...
    disabledDate() {
      ...
      return {
        disabledDate(time) {
          let flag = that.mostDay && that.mostDay != 0;
          // console.log(that.limitTime);
          let limitDay = parseInt(that.limitDay);
          let mostDay = (that.mostDay && parseInt(that.mostDay)) || 0;
          ...
          if (flag) {
            let mostDate = new Date(
              new Date().setDate(new Date().getDate() + parseInt(mostDay))
            ).getTime();
            return (
              time.getTime() <
                new Date(
              ...
              time.getTime() >
                new Date(
                  that.util.formatTime(mostDate, "YYYY-MM-DD 00:00:00")
                ).getTime()
            );
          }
          return (
  ...
  created() {
    ...
    let obj =
      this.timeOption.find(t => this.form.daytype == t.value) ||
      this.timeOption[0];
    this.selectTime = obj.text;
    this.selectTimeId = obj.value;
    ...
    this.common.findRule("IRAPPLYDAYMAX").then(res => {
      if (res.success) {
        this.mostDay = res.item.rulevalue;
