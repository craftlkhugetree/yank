<template>
  <div class="my">
    ...
    >
      <!--    盒子动画效果-->
      <transition name="fade" v-show="!item.isExpand" :duration="1000">
        <!-- <div v-show="!item.isExpand"> -->
        <!-- :class="item.isExpand ? 'select-close' : 'select-open'" -->
        <div v-show="!item.isExpand">
      ...
      <!------------------------- 详情 ------------------------->
      <transition name="fadeDetail" v-show="item.isExpand" :duration="1000">
        <detail
...
export default {
  ...
  methods: {
    findById(id) {
      let name = "detail" + id;
      const that = this.$refs[name][0];
      findId(id)
        .then(res => {
          if (res && res.success) {
            let rInfo = { ...res.data };
            that.statusName = repairStatus.find(
              r => r.val == rInfo.status
            ).name;
            // console.log(this.$refs[name], this.$refs.detail);
            that.rInfo = { ...rInfo };
            that.transPhotos(rInfo, that.imgs);
            that.genProgress();
            that.$refs.judge && that.$refs.judge.genR();
    ...
    toDetail(row) {
      ...
      //   timer = setInterval(() => {
      //     if (this.$refs.detail) {
      //       this.$refs.detail[0].$refs.judge &&
      //         this.$refs.detail[0].$refs.judge.genR();
      //       clearInterval(timer);
      //     }
      //   }, 500);
      // });
