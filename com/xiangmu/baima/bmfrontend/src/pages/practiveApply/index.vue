<template>
  <div class="common-content">
      ...
      <el-table-column label="操作" fixed="right" width="200" align="center">
        <template slot-scope="scope">
          ...
          <div
            ...
            @click="operateApply('reAdd', scope.row)"
            v-if="(scope.row.applystatus == '7' || scope.row.applystatus == '8' || scope.row.applystatus == '9')"
          >
            重新申请
          </div>
        </template>
    ...
    <el-dialog
      ...
      :title="dialogType == 'add' ? '实习申请新增' : '实习申请编辑'"
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      showViewer: false,
      srcList: [require("st@tic/images/help-bkssx.png")]
    };
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    prTimeFormatter(row, column, cellValue) {
      let startDate = cellValue
        ? this.util.formatTime(cellValue, "yyyy.MM.dd")
        : "--";
      let endDate = row.prendtime
        ? this.util.formatTime(row.prendtime, "yyyy.MM.dd")
        : "--";
    ...
    operateApply(type, row) {
      ...
      this.form = {
        ...
        orgname: this.options.userInfo.ORGNAME,
    ...
    triggerSubmit(type) {
      ...
      this.$refs.child.diagBody =
        "实习期间，未经允许，请勿随意进入他人实验区域。";
    },
        ...
        this.$refs.child.$refs.form.validate(valid => {
          if (valid) {
            ...
            let teacherList = this.$refs.child.teacherList;
              ...
              if (
                form.prstarttime > form.prendtime
              ) {
                this.$message({
                  type: "warning",
                  message: "实习开始日期不能晚于结束日期"
                });
                return
              } else if (
                form.issleep == "1" &&
            ...
            if (teacherList && teacherList.length == 0) {
              ...
              //日期校验
                ...
                form.issleep == "1" &&
                form.sleepstarttime < form.prstarttime
              ) {
                this.$message({
                  type: "warning",
                ...
                form.iseat == "1" &&
                (!form.eatstarttime || !form.eatendtime)
              ) {
                this.$message({
                  type: "warning",
                  message: "请选择用餐日期"
