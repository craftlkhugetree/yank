  ...
  <div class="common-content" v-loading="loading">
    ...
    >
      <apply-form
        ...
        @submitDiag="innerSubmit(dialogType)"
        :notRent="true"
      ></apply-form>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-tooltip
          ...
          placement="top"
        >
          <div class="my-button green" @click="submit('add')">
            提 交
          </div>
        </el-tooltip>
      </div>
...
export default {
  ...
  components: {
    ...
    applyForm: () => import("./resApplyDialog2")
  },
  data() {
    return {
  ...
  methods: {
    openHistory() {
      this.form2 = {
        orgId: "", // "000029",
        orgName: "", // "农学院",
        classfeeLeaderName: "", // "农教职工-2",
        classfeeLeader: "", // "16000-2",
        eduTypeId: "", // 17,
        billingCycle: "", // 2,
        billingMethod: "", // 1,
        eduTypeName: "", // "卫岗-智能温室",
        eduResourceIds: "", // [88],
        areas: "", // "36.73",
        rules: "", //"1.智能温室管理依照《南京农业大学卫岗校区智能温室使用管理暂行办法》执行，乙方如不能履行协议，甲方有权收回温室，并向乙方收取违约金。\n2.乙方不得进行转基因相关实验。（转基因实验须另行申请到特定区域开展）\n3.乙方擅自转租、转让、转借资源，甲方有权收回。\n4.乙方在使用期间，不得随意搭建、硬化地面，负责 安全管理和环境卫生整治。\n5.为提高资源利用率，乙方签订协议后，如资源长期闲置不用的，甲方有权收回。\n6.甲方有权根据学校发展需要修订相关管理条款，乙方须按照甲方修订后的管理规定执行。",
        ct2: "", // "m²",
        chargecycle: "", // "月",
        ct1: "", // "面积"
        rentStartTime: null
        ...
        .then(res => {
          ...
          if (res.success == true) {
            ...
            this.resList.forEach(r => {
              ...
              let chargecycle =
                (r.billingCycle || (r.eduType && r.eduType.billingCycle)) + "";
              let chargetype =
                (r.billingMethod || (r.eduType && r.eduType.billingMethod)) +
                "";
              this.common.chargecycleFormatter(chargecycle, r);
              this.common.chargetypeFormatter2(chargetype, r, "ct2", "ct1");
  ...
  created() {
    this.getResTypeList();
    if (!JSON.parse(sessionStorage.getItem("groupList"))) {
