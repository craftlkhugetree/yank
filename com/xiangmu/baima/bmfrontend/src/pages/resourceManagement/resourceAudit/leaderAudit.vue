    ...
    >
      ...
      <el-table-column label="操作" fixed="right" width="150" align="center">
        <template slot-scope="scope">
          ...
          <div
            v-if="scope.row.handleNode == 'LD' && scope.row.status == 1"
            class="table-btn blue"
            @click="goAudit(scope.row)"
          >
            审批
          </div>
          <!--            <div v-if="(scope.row.usetype == '3') && scope.row.actionstatus=='1' && scope.row.applystatus ==3 " class="table-btn pink" @click="reCall(scope.row)">退租</div>-->
        </template>
      </el-table-column>
    </el-table>
