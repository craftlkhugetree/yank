<template>
  <div class="common-content">
    ...
    >
      ...
      ></el-table-column>
      <el-table-column
        v-if="resTypeDetail.chargecycle"
        prop="price"
        :label="
          '单价（元/' +
          ...
          <div
            v-if="
              scope.row.usestatus == '3' &&
                !scope.row.hasXuzApply &&
                !scope.row.hasExitApply &&
                scope.row.liveinfo &&
                scope.row.liveinfo.limitDay <= 30
            "
    ...
    <!--报修弹框-->
    <el-dialog
      class="res-apply-dialog repair-dialog"
      title="报修"
      v-if="repairFormVisible"
          ...
          if (res.success == true) {
            ...
            this.resList.forEach(v => {
              if (v.liveinfo && v.liveinfo.applyendtime) {
                v.liveinfo.applyendtime = this.util.formatTime(
                  v.liveinfo.applyendtime,
                  "yyyy.MM.dd hh:mm:ss"
                );
              }
            });
      ...
      this.util
        ...
        .then(res => {
          ...
          if (res.success == true) {
            ...
            this.activeResType =
              sessionStorage.getItem("activeResType") || this.resTypeList[0].id;
