  ...
  methods: {
    ...
    getLiveList(page) {
      return new Promise((resolve, reject) => {
        ...
        if (this.isUser) {
          filter.owner = this.$store.state.userId;
        }
        const params = {
      ...
      eduResourceReCheckin({ id })
        .then(res => {
          if (res && res.success) {
            ...
            this.form.eduResourceIds = [parseInt(id)];
            this.form.areas = this.resTypeForm.area;
            this.form.classfeeLeader = this.$store.state.userInfo.ID;
            this.form.classfeeLeaderName = this.$store.state.userInfo.NAME;
