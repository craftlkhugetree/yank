      ...
      <el-table-column label="操作" fixed="right" width="300" align="center">
        <template slot-scope="scope">
          ...
          <div
            ...
            v-if="
              (scope.row.applystatus === '2' && !isRearService) ||
...
export default {
  ...
  watch: {
    ...
    '$route.path'(o, n) {
      // console.log(o, n, this.$route.path);
      this.getList(1)
    }
  },
  methods: {
    ...
    downloadForm(type, row) {
      ...
      let isSameDay =
        this.resDetail.eatstarttime === this.resDetail.eatendtime;
      let eatObj = {
        "1,1": "含早餐",
        "2,2": "含中餐",
        "3,3": "含晚餐",
      ...
      if (isSameDay) {
        ...
        this.resDetail.eatstarttype = this.resDetail.eatendtype =
          eatObj[str];
      } else {
        if (eatstarttype) {
          switch (eatstarttype) {
      ...
      this.util
        ...
        .then(res => {
          ...
          if (res.success == true) {
            ...
            this.auditList = (res.item[str] && res.item[str].list) || [];
            this.totalPage = (res.item[str] && res.item[str].total) || 0;
