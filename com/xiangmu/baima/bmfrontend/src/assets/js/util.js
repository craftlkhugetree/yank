...
global.url["appportal"] = window.g.ApiUrl1;
global.url["auth"] = window.g.ApiUrl;
global.url["bm"] = window.g.ApiUrl2;
global.url["file"] = window.g.ApiUrl3;
global.url["comm"] = window.g.ApiUrl4;
      ...
      getRealUrl(options.code, options.url)
        .then(function(data) {
          let ajaxSettings = {
            ...
            headers: { IDSTGC: getCookie('IDSTGC') || '59386188698f47d5a7c4b16c6278d14b'},
            data: options.isRep
              ? options.data
          ...
          return Axios(ajaxSettings)
            .then(function(data) {
              ...
              if (
                res.login === false ||
                (typeof res === "string" && res.includes("统一认证"))
              ) {
                redirectToLogin();
              } else {
            ...
            .catch(function(data) {
              ...
              // IE浏览器
              // if (appCode === "Mozilla" && (browser === "Microsoft Internet Explorer" || browser === "Netscape")) {
              //   redirectToLogin();
              // }
              reject(data);
            });
