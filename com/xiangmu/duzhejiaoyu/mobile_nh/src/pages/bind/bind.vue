  ...
  <div>
      ...
      <div class="bindcontent_bottom">
        ...
        <p>
          ...
          <input
            type="text"
            v-model="loginname"
            :placeholder="placeholderCardText"
            value
            id="loginname"
          />
        </p>
        <p>
          <i>密码</i>
          <input :placeholder="placeholderPassText" type="password" v-model="pwd" value id="name" />
        </p>
        <div class="valid_num">
          <i>验证码</i>
        ...
        <button class="userbind_bottom" @click="bind" id="bindlogin">登录</button>
        <p
          v-if="nh && isnewlogin"
          class="bind_info"
          style="font-size: .7rem;text-align: center;color: red;margin-top: .5rem;"
        >({{nhLoginInfo}})</p>
        <div class="newlogin" @click="gonewStulogin" v-if="isnewlogin">
          本科新生登录
          <i></i>
        </div>
      </div>
    ...
    <div class="guide_meng" v-if="guide_show&&isnewlogin">
      <div class="guide_meng_">
        <div class="guide_meng_btn">
          <div class="guide_login" v-if="isnewlogin" @click="bind">
            登录
            <i></i>
          </div>
          <div class="guide_newlogin" @click="gonewStulogin" v-if="isnewlogin">
            本科新生登录
            <i></i>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    bind() {
      if (this.guide_show && this.isnewlogin) {
        this.guide_show = false;
        return;
      }
      if (this.loginname == "") {
