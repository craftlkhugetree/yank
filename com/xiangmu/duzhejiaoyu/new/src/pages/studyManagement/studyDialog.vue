  ...
  <div>
    ...
    >
      ...
      <el-form-item label="资料图片" prop="photourl">
        ...
        <div v-if="form.photourl && form.photourl != '-1'">
          <img
            :src="domain + form.photourl"
      ...
      <el-form-item label="附件" prop="url" v-if="form.fileType != 3">
        <el-button type="primary" plain size="small" @click="uploadFile('file')"
          >上传附件</el-button
        >
        <span style="color: red" v-if="form.fileType == 1"
          >(可上传格式有 mpeg,mpg,dat,mp4,mkv)</span
        >
        <span style="color: red" v-if="form.fileType == 2"
          >(可上传格式有 pdf,doc,docx,xls,xlsx,ppt,pptx)</span
        >

        <div>
          {{
            this.form.url
              ? this.form.url.split("/")[this.form.url.split("/").length - 1]
              : ""
          }}
        </div>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      option: {
        uploadJson: window.g.activityimgsaveURL, //上传地址
        fileReadUrl: window.g.activityimgURL, //读取文件的地址
        ...
        templateCode: this.global.code //模板分类码
      },
  ...
  methods: {
    ...
    submit() {
      ...
      console.log(this.$refs["editor1"]);
      let form = {
        fileType: this.form.fileType,
        name: this.form.name,
        campuss: arr,
        // description:this.form.description,
        description: this.form.fileType == 1 ? this.$refs["editor"] ? this.$refs["editor"].getHTML() : "" : this.form.description,

        // dhtml:this.form.dhtml,
        dhtml: this.$refs["editor1"] ? this.$refs["editor1"].getHTML() : "",
        url: this.form.url,
        id: this.form.id,
    ...
    confirm() {
      this.form.dhtml = this.$refs["editor1"]
        ? this.$refs["editor1"].getHTML()
        : "";
