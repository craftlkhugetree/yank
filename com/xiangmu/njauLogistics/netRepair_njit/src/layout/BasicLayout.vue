<template>
  <el-container>
    ...
    <el-header>
      <div class="header">
        <img src="@/../static/images/school2.png" alt />
        <div class="header-menu" v-if="menu.length > 1">
          <router-link
            :class="{
              'router-link-active':
                item.DISPLAYURL.split('/')[1] === $route.path.split('/')[1]
            }"
            v-for="item in menu"
            :key="item.ID"
            :to="item.DISPLAYURL"
    ...
    <!------------------------------ 底部 ------------------------------>
    <el-footer>南京工程学院版权所有 Copyright © 2022</el-footer>
  </el-container>
</template>
  ...
  methods: {
    ...
    exit() {
      ...
      document.cookie =
        ...
        // "IDSTGC=" + escape("echo") + ";expires=" + expires.toGMTString();
      ...
      let toUrl =
        ...
        (lastUrl ? lastUrl : location.protocol + "//" + url);
      location.href = toUrl;
    }
  }
};
