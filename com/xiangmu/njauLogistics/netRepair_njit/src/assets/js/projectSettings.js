...
let devAppportalurl = 'http://seat.dev.angke.cn/appportalweb/rest/';
// 设置登录的url，如果是PC端则不用填写，手机端或者需要自定义绑定页面则需要填写。
let loginUrl = '';
// let projectName = "/unitedfront";
...
let loginurl = function (loginUrl) {
    if(loginUrl){
        return loginUrl
    }else{
        ...
        let purl=document.location.href.split("#")[0].replace(/&/g,"___");
        let lastUrl = sessionStorage.getItem("angkeNowUrl");
        sessionStorage.removeItem("angkeNowUrl");
        return (lastUrl ? lastUrl : 
          location.protocol + '//' +
          location.host +
          "/appportalweb/rest/Idsclient/reqLogin?reqUrl=" + encodeURIComponent(purl)
        );
        // return window.g.url_2 + 'rest/Idsclient/reqLogin?reqUrl='+encodeURIComponent(purl);
