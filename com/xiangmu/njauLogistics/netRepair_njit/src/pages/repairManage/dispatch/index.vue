  ...
  <div style="min-height:800px;">
    ...
    <transition class="container" name="slide-fade">
      <el-row v-if="activeTab==='1'">
        <el-col :span="12">
          <list key="1" ref="list" v-if="userId" :params="listParams" canSelect></list>
        </el-col>
        <!------------------------- 待派单详情 ------------------------->
        <el-col :span="12">
          ...
          <div class="status-box clearfix">
            ...
            >
              <div class="status-box-img">
                <img :src="applystatus == item.status ? item.imgA : item.img" alt />
                <span :class="{'active':applystatus == item.status}">{{item.num}}</span>
              </div>
              {{item.name}}
            </div>
          </div>
    ...
    <!---------------------------- 批量办理弹窗 ---------------------------->
    <take-out-drawer ref="takeOutDrawer" @doFunc="$refs.list.getList();getTabNum();"></take-out-drawer>
    <!---------------------------- 批量退回弹窗 ---------------------------->
    <back-drawer ref="backDrawer" @doFunc="$refs.list.getList();getTabNum();"></back-drawer>
...
export default {
  ...
  methods: {
    ...
    handleBatch(type) {
      ...
      let drawer = type == "takeOut" ? this.$refs.takeOutDrawer : this.$refs.backDrawer;
    ...
    // 获取tab数字
    getTabNum() {
      if (!(this.userId && this.userDept && this.userDept.ID)) return
      // 获取待派单数据
      this.getNum({
        applystatus: "2",
        repairdeptid: this.userDept.ID || "",
