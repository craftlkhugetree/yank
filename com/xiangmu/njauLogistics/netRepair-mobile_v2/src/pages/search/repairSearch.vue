<template>
  <div>
    <van-nav-bar title="维修查询" left-arrow fixed @click-left="$router.go(-1)" />
          ...
          <div slot="states">
            <div class="orderbtns clearfix">
              <!-- <span class="orderstate">
                {{ item.status != 1 ? "维修完工" : "未维修" }}
              </span> -->
              <div class="btns">
                <!-- <span class="btn gray">评价</span> -->
                <span class="btn" @click.stop="toDetail(item)">查看更多</span>
              </div>
            </div>
          </div>
        </listItem>
        <emptyList class="emptylist animation-1" v-show="!results.length && !loading"></emptyList>
      </van-list>
    </div>
  </div>
</template>
...
import listItem from "../maintenance/maintenanceList/listItem";
