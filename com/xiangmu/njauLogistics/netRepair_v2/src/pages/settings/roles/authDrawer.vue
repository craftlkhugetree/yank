      ...
      this.util
        ...
        .then(res => {
          ...
          this.authList.forEach((i, index) => {
            // 四个角色默认权限不可取消
            // 接报修人员9100002njit-2：报修管理-维修管理9100002njit-2、报修录单9100002njit-3
            if (this.roleId === "9100002njit-2" && i.ID === "9100002njit-2") {
              i.ISDEFAULT = true;
              i.items.forEach((j, index) => {
                if (["9100002njit-2", "9100002njit-3"].includes(j.RESID)) {
                  j.ISDEFAULT = true;
                }
              });
            }
            // 维修责任人9100002njit-3：报修管理-派单管理9100002njit-4
            if (this.roleId === "9100002njit-3" && i.ID === "9100002njit-2") {
              i.ISDEFAULT = true;
              i.items.forEach((j, index) => {
                if (j.RESID === "9100002njit-4") {
                  j.ISDEFAULT = true;
                }
              });
            }
            // 全校师生 默认菜单 首页
            if (this.roleId === "9100002njit-1" && i.RESID === "9100002njit-1") {
              i.ISDEFAULT = true;
            }
            // 维修工 默认菜单 我的维修
            if (this.roleId === "9100002njit-4" && i.ID === "9100002njit-7") {
              i.ISDEFAULT = true;
              i.items.forEach((j, index) => {
                if (["9100002njit-j"].includes(j.RESID)) {
                  j.ISDEFAULT = true;
                }
              });
            }
