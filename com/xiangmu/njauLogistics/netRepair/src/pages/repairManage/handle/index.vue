  ...
  <div style="min-height:800px;">
    ...
    <transition class="container" name="slide-fade">
      <el-row ref="repairhandle" v-if="activeTab==='1'">
        <el-col :span="12">
          <list key="1" ref="list" :params="listParams" canSelect></list>
        </el-col>
        <!------------------------- 待办理详情 ------------------------->
        <el-col :span="12">
    ...
    <transition class="container" name="slide-fade1">
      <el-row ref="repairhandle" v-if="activeTab==='3'">
        <el-col :span="12">
          <list key="3" ref="list" :params="listParams"></list>
        </el-col>
        <!------------------------- 待办理/已办理详情 ------------------------->
        <el-col :span="12">
    ...
    <!------------------------- 办理弹窗 ------------------------->
    <take-out-drawer ref="takeOutDrawer" :repairLeaderList="repairLeaderList" @doFunc="$refs.list.getList();getTabNum();"></take-out-drawer>
    <!------------------------- 转移弹窗 ------------------------->
    <move-drawer ref="moveDrawer" :deptList="deptList" @doFunc="$refs.list.getList();getTabNum();"></move-drawer>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleBatch(type) {
      ...
      let drawer = type == "takeOut" ? this.$refs.takeOutDrawer : this.$refs.moveDrawer;
