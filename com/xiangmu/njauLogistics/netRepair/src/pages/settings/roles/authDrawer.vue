      ...
      this.util
        ...
        .then(res => {
          ...
          this.authList.forEach((i, index) => {
            ...
            if (this.editForm.id === "9100002-2" && i.ID === "9100002-2") {
              i.ISDEFAULT = true;
              i.items.forEach((j, index) => {
                if (["9100002-2", "9100002-3"].includes(j.RESID)) {
                  j.ISDEFAULT = true;
                }
              });
            }
            // 维修责任人9100002-3：报修管理-派单管理9100002-4
            if (this.editForm.id === "9100002-3" && i.ID === "9100002-2") {
              i.ISDEFAULT = true;
              i.items.forEach((j, index) => {
                if (j.RESID === "9100002-4") {
                  j.ISDEFAULT = true;
                }
              });
            }
            // 维修共9100002-4：我的维修-9100002-8
            if (this.editForm.id === "9100002-4" && i.ID === "9100002-8") {
              i.ISDEFAULT = true;
              i.items.forEach((j, index) => {
                if (j.RESID === "9100002-k") {
                  j.ISDEFAULT = true;
                }
              });
            }
            // 全校师生 默认菜单 首页
            if (this.editForm.id === "9100002-1" && i.RESID === "9100002-1") {
              i.ISDEFAULT = true;
            }
