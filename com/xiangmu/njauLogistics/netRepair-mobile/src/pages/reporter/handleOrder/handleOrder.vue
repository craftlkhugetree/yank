<template>
  <div>
    ...
    <orderInfo :info="info" v-if="info.createtime" class="orderInfo"></orderInfo>
    <div v-if="!showHandle" class="otherdept">维修部门：{{ repairDeptName }}</div>
    <div v-if="showHandle">
      <!-- 接报修 -->
      <div class="handler" v-if="info.applystatus === '1'">
        <van-button class="halfbtn" type="info" @click="openHandle">办理</van-button>
        <van-button class="halfbtn" plain type="info" @click="openMove">转移</van-button>
      </div>

      <div class="handler" v-if="info.applystatus === '2'">
        <van-button class="fullbtn" type="info" @click="toWithDraw">撤回</van-button>
      </div>
    </div>
    <h2>
      报修单进展
    ...
    <!-- 维修负责人选择 -->
    <van-popup v-model="repairleaderPick" position="bottom" :style="{ height: '40%' }">
      <van-picker
        value-key="NAME"
        show-toolbar
    ...
    >
      ...
      <van-form ref="moveForm" :show-error-message="false">
        ...
        <van-field v-model="note" label="转移描述" :rules="[{ required: true, message: '请输入说明，不超过200字' }]" />
        <van-field
          type="textarea"
          :rows="3"
          v-model="note"
            ...
            this.util
              ...
              .then(res => {
                if (res.success) {
                  ...
                  this.repairDeptName =
                    res.items[
                      _.findIndex(res.items, { ID: repairdeptid })
                    ].NAME;
                }
              });
