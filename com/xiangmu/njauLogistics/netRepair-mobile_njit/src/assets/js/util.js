...
var IDSTGC = "0e5375875b7645968cdec8495a29744f";
// var IDSTGC = "0e5375875b7645968cdec8495a29744f";
          ...
          return Axios(ajaxSettings)
            .then(function(data) {
              ...
              if (
                ...
                console.log(query);
                if (query && query.source === 'wx') {
                  location.href =
                    "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxf302b6fafc6c6108&response_type=code&scope=snsapi_userinfo&state=123&redirect_uri=https%3A%2F%2Fbaoxiu.njit.edu.cn%2Fmnjitrepair%2Flogin";
                } else {
                  redirectToLogin();
                }
              } else {
                if (res.success === false) {
                  ...
                  let msg =
                    ...
                    orgindata.msg ||
                    orgindata.message;
                  if (msg && options.loadingText !== null) {
                    thisVue.$toast.fail(msg);
                  }
                  resolve(orgindata);
                } else {
                  resolve(res);
...
let formatTime = function(time, format) {
  ...
  switch (timeLength.length) {
    ...
    default:
      ...
      console.log("format参数输入错误,输入为 ：" + time);
  }
  let timeString = "";
  if (dateObj) {
    timeString = format
...
export default {
  ...
  regExps: {
    ...
    qq: /^[1-9]\d{4,9}$/, // QQ
    phone: /^((0\d{2,3}-\d{7,8})|(1[3584679]\d{9})|(\d{8}))$/,
