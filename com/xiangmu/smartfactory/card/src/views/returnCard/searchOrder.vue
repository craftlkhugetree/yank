<template>
  <div class="order">
    ...
    <!----------------------------- 打印中。。。 ----------------------------->
    <searching v-if="printing && !signing" :src="require('@/assets/img/print.png')" :text="printingText"></searching>
	<!----------------------------签字版------------------------------->
    ...
    <div class="table-box noPrint" v-if="!loading && !printing && order.id&&!signing">
      ...
      <table border="1" width="100%">
        ...
        <tr>
          <td>实装量</td>
          <td>{{order.actQty ? order.actQty + "KG" : "--"}}</td>
        </tr>
        <tr>
          <td>客户名称</td>
		...
		<tr>
		  <td>司机</td>
		  <td>{{order.drivers[0].name+(order.drivers.length>1?'，'+order.drivers[1].name:'')}}</td>
		</tr>
...
export default {
  ...
  methods: {
	...
	/**this.order = {
          ...
          goodsName: '甲苯',
          shipname: '苏A0003',
          drivers: [{ id: null, billId: null, name: '徐宇鹏', idcard: '320924198910286513' },{ id: null, billId: null, name: '徐明华', idcard: '320924198910286513' }]
        };
	return true;**/
      ...
      searchOrderByCard({
        ...
        .then(res => {
          if (res.code == '000000' && res.data && res.data.id&&res.data.status=='5') {
            ...
            this.order = res.data || {};
			//this.order.actQty = maoWeight.weight
          } else {
	...
	signComplete(){
		let signStream = 'data:image/png;base64,'+this.HWPenSign.HWGetBase64Stream(2);
		...
		//this.order = {...this.order,shipname:'gegergre'}
		document.getElementById('imgurlvalue').src=signStream;
		//this.$refs.PrintBox1.signStream=signStream;
		this.HWPenSign.HWFinalize();
		this.showBtns = false;
		this.signing = false;
		this.printOrder();
