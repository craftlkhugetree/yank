  ...
  <div class="main">
    ...
    <s-card ref="acadeTable" :params="params" :getList="QUERY" :resHook="resHook" :limit="limit" isSecondPage>
      <div
        ...
        :key="item.id"
        class="publishbook"
        :class="hasSth(1) || hasSth(2) ? 'w25' : hasSth(4) ? 'w50' : ''"
      >
        <publish-book :data="item" :notPaper="true" v-if="hasSth(0)"></publish-book>
        <paper :data="item" date="发表日期：" v-if="hasSth(1)"></paper>
        <paper :data="item" date="获得日期：" v-if="hasSth(2)"></paper>
        <publish-book :data="item" :notPaper="false" v-if="hasSth(3)"></publish-book>
        <honor :data="item" :keyArr="honorArr" v-if="hasSth(4)" :titleLine="3"></honor>
      </div>
    </s-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    clickIcon(item) {
      this.active = item;
      if (this.hasSth(1) || this.hasSth(2)) {
        this.limit = 8;
      } else if (this.hasSth(4)) {
        this.limit = 6;
      } else {
        this.limit = 3;
      }
