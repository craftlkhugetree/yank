  ...
  methods: {
    ...
    getFile(e) {
      ...
      let axiosSettings = {
        url: this.url,
        method: 'POST',
        headers: { IDSTGC: this.getCookie('IDSTGC') || this.util.IDSTGC },
        data: formData,
      };
      if (_this.videoUploadPercent) {
        ...
        axiosSettings.onDownloadProgress = function(progressEvent) {
          ...
          if (progressEvent.lengthComputable) {
            ...
            _this.videoUploadPercent((progressEvent.loaded / progressEvent.total) * 100); //实时获取最新下载进度
            // _this.$emit('videoUploadPercent', (progressEvent.loaded / progressEvent.total) * 100); //实时获取最新下载进度
          }
        };
      }
