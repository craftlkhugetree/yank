...
var IDSTGC = '';
// var IDSTGC = '603d198673b142d4959bab2275ed987e';
  ...
  return new Promise(function(resolve, reject) {
    if (options && options.url && options.code) {
      ...
      if (options.loadingText || options.loadingText === '') {
        // console.log(thisVue);
        loading = thisVue && thisVue.$loading && thisVue.$loading({
          lock: true,
      ...
      getRealUrl(options.code, options.url)
        .then(function(data) {
          let ajaxSettings = {
            ...
            headers: { IDSTGC: getCookie('IDSTGC') || IDSTGC },
          ...
          return Axios(ajaxSettings)
            .then(function(data) {
              ...
              if (res.login === false || (typeof res === 'string' && res.includes('<html>'))) {
                ...
                if (res.success === false || (res.code && res.code !== '000000')) {
                  // if(!res.errInf || res.errInf.metailBusInf.indexOf('错误唯一流水码') != -1){
                  //   setting.tips(res.message);
                  // }
                  let msg = res.message || res.msg || orgindata.msg || orgindata.message;
                  msg && thisVue.$message({
                    showClose: true,
                    type: 'error',
                    message: msg,
