    ...
    <div class="drawer-container">
      ...
      >
        <el-form-item
          ...
          :key="item"
        >
          <div v-if="hasSth(item, '年代')" class="date_div_flex">
            <el-date-picker
...
export default {
  ...
  data() {
    const validateDate = (rule, value, callback) => {
      if (!this.editForm.publishYear) {
        callback(new Error('请选择年代'));
      } else {
        callback();
      }
    };
    return {
      ...
      rules: {
        ...
        studentName: [{ required: true, message: '请输入硕士博士姓名', trigger: 'blur' }],
        publisher: [{ required: true, message: '请输入出版社', trigger: 'blur' }],
        publishYear: [{ validator: validateDate, trigger: 'blur', required: true }],
      },
    ...
    saveAccount() {
      this.$refs.editForm.validate(valid => {
        if (valid) {
          ...
          let param = { relationship: this.rs, personId: this.personId };
