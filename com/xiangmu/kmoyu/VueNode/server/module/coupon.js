...
router.get('/home', async (req, res) => {
  try {
    const { regionId, classifyId, page, order, sort } = req.query 
    let orderSql = 'ORDER BY ' + (sort ? sort + ' ' + order.toUpperCase(): 'coupon_starttime')
    let couponList = []
    if (+regionId === 1) {
      couponList = await db(
        'select id, coupon_starttime, coupon_endtime, coupon_name, coupon_explain, coupon_ico_path, coupon_status, coupon_recived_num from tour_coupon where coupon_classify="' +
          classifyId +
          `" ${orderSql}` +
          ' limit ' +
          (page - 1) * 10 +
      ...
      couponList = await db(
        'select id, coupon_starttime, coupon_endtime, coupon_name, coupon_explain, coupon_ico_path, coupon_status, coupon_recived_num from tour_coupon where coupon_belong_region="' +
          regionId +
          '" and coupon_classify="' +
          classifyId +
          '" limit ' +
