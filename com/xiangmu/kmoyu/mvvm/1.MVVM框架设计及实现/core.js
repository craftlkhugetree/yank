...
(function(){
	...
	MVVM.define = function(name, factory) {
		...
		//收集所有定义
		factory(scope);
		console.log('scope上有传入的data属性:', scope);

		//生成带get set控制器与自定义事件能力的vm对象
		var model = modelFactory(scope);

		//改变函数引用为转化后的vm对象,而不再是scope对象,这里就触发了set
		stopRepeatAssign = true
		factory(model) // model.w = 150
	...
	function modelFactory(scope) {
		...
		console.log('aaObserv:', aaObserver, this, vModel);
	...
	function resolveAccess(name, val, originalModel, normalProperties, accessingProperties, watchProperties) {
		...
		if (valueType === 'number') {
			...
			accessor = function(newValue){
				...
				if (arguments.length) { //set
					if (stopRepeatAssign) {
						return //阻止define里的第二个factory()触发set，重复赋值
					}
					//确定是新设置值
		...
		console.log('acc:', accessingProperties);
	}
	...
	function notifySubscribers(accessor) {
		...
		if (list && list.length) {
			...
			console.log('依赖列表：', list, arguments, args);
	...
	function withValue(access) {
	    ...
	    for (var i in access) {
	        descriptors[i] = {
				// get和set用了同一个函数
				get          : access[i], // 含get、set的accessor函数
				set          : access[i],
				enumerable   : true,
				configurable : true
	        }
	...
	// code是变量名，scopes是含有get、set的总对象 VMODELS['box']。data是含有get、set的具体属性
	...
	function parseExpr(code, scopes, data){
		...
		console.log('生成eval, fn:', fn, code, data);
		//生成求值函数
		data.evaluator = fn;
	}
    ...
    function registerSubscriber(data) {
      ...
      // dataType为click的，true则直接return，而不执行evaluator，也就是fn
      MVVM.openComputedCollect = true; //排除事件处理函数
      var fn = data.evaluator;
      console.log('依赖收集与触发', data, fn);
      if (fn) {
        //如果是求值函数，handler就是更新DOM，执行完就可以delete掉了。也就不push到监听数组中collectSubscribers
        // 执行了fn，里面触发了变量的get，收集了依赖，同时全局Registry[expose]就是该变量，所以可以被收集到。
