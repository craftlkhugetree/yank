/* 创建者：shuxiaokai 创建时间：2020-09-21 22:16 模块名称：账号密码登录 备注：xxxx */
<template>
  <el-form
    ref="form"
    class="login-account"
  ...
  >
    ...
    <el-form-item v-if="config.localization.enableGuest" class="mb-1">
      <el-button :loading="loading" class="w-100" type="primary" @click="handleGuesttLogin">
        {{ $t('直接登录(体验账号，数据不会被保存)') }}
      </el-button>
    </el-form-item>
    <el-form-item class="mb-1">
      <el-button
        :loading="loading"
        :type="config.localization.enableGuest ? '' : 'primary'"
        native-type="submit"
        class="w-100"
      >
        {{ $t('登录') }}
    ...
    <div class="forget-pwd-wrap">
      <el-button text link type="primary" @click="handleJumpToResetPassword">
        {{ $t('已有账号，忘记密码?') }}
      </el-button>
    </div>
    <div v-if="config.localization.enableDocLink" class="mt-2 d-flex j-around">
      <a
        href="https://github.com/trueleaf/moyu"
        target="_blank"
        class="d-flex flex-column j-center a-center"
      >
      ...
      <a
        href="https://gitee.com/shuzhikai/moyu"
        target="_blank"
        class="d-flex flex-column j-center a-center"
      >
      ...
      <a
        href="https://www.yuque.com/happymoyu/as0gig"
        target="_blank"
        class="d-flex flex-column j-center a-center"
      >
      ...
      <a
        :href="config.localization.download.url"
        target="__blank"
...
export default defineComponent({
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.form.validate((valid: boolean) => {
        if (valid) {
          this.loading = true;
          this.axios
            .post<Response<PermissionUserInfo>, Response<PermissionUserInfo>>(
    ...
    handleGuesttLogin() {
      this.loading = true;
      this.axios
        .post('/api/security/login_guest', this.userInfo)
        .then(res => {
          this.$router.push('/v1/apidoc/doc-list');
          localStorage.setItem('userInfo', JSON.stringify(res.data));
        })
