...
export default defineComponent({
  ...
  created() {
    this.getProjectList();
    this.initCahce();
    console.log(dayjs(new Date()).format('YYYY-MM-DD'), dayjs.foo)
    // dayjs.foo = (d) => d
    // dayjs.fooooo = (d) => d
  },
  methods: {
    ...
    getProjectList() {
      this.loading = true;
      this.axios.get<Response<ApidocProjectListInfo>, Response<ApidocProjectListInfo>>('/api/project/project_list', { data: 123 }).then((res) => {
    ...
    deleteProject(_id: string) {
      this.$confirm(this.$t('此操作将永久删除此条记录, 是否继续?'), this.$t('提示'), {
        ...
        this.axios.delete('/api/project/delete_project', { data: params, params: { test: 555 } }).then(() => {
          this.getProjectList();
        }).catch((err) => {
          console.error(err);
        });
