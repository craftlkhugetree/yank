<template>
  <div class="audit">
    <van-nav-bar title="月度绩效确认" fixed :border="false" left-arrow @click-left="$router.goBack()" />
...
export default {
  data() {
    return {
      activeTab: sessionStorage.getItem("monthConfirmTab") || "1",
      tabs: [
        {
          id: "1",
          title: "待确认"
  ...
  methods: {
    ...
    goDetail(item) {
      ...
      sessionStorage.setItem("monthConfirmDetail", JSON.stringify(item));
    },
    // 获取列表
    getList() {
      ...
      func({
        ...
        if (res.success) {
          sessionStorage.setItem("monthConfirmTab", this.activeTab);
          this.list = res.data || [];
          if (this.activeTab == "2") {
            this.list.sort((a, b) => {
