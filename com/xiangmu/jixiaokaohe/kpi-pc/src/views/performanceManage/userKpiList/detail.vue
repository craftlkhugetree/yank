<template>
  <div class="base-search-table" v-loading="loading">
    ...
    <div class="search-box clearfix" style="padding: 20px 20px 0;">
      <i class="el-icon-arrow-left" @click="$router.push('/kpi/user-kpi-list')">返回</i>
...
import { fetchUserKpiDetail } from "@/api/kpi/userKpi.js";
export default {
  props: {
    userid: String,
    year: String
  ...
  computed: {
    title() {
      return `『 ${this.detail.name}（${this.detail.id}） 』${this.curYear}年绩效记录`;
    },
    total() {
  ...
  methods: {
    ...
    getUserKpi() {
      ...
      let param = {
        userid: this.userid,
        yeardate: this.curYear
      };
      fetchUserKpiDetail(param)
        .then(res => {
          this.loading = false;
          if (res.success) {
            let data = res.data || [];
