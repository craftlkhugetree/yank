...
// var IDSTGC = "";
var IDSTGC = "a968493276454e90914adf49923b396e";
...
let startAjax = function (options) {
  return new Promise(function (resolve, reject) {
    ...
    if (options && options.url && options.code) {
      load_unload(options.ifLoad, true);
      getRealUrl(options.code, options.url)
        .then(function (data) {
          ...
          return Axios(ajaxSettings)
            .then(function (data) {
              load_unload(options.ifLoad, false);
              let orgindata = data;
              let res = data.data;
...
/** 某些接口默认转圈圈 */
const load_unload = function (ifLoad, bool) {
  ifLoad && store.commit('setLoading', bool);
};
