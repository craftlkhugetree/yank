<template>
  <div class="food" v-loading="typeLoading">
    <div class="cafe-type">
      <label>分类：</label>
    ...
    <div class="cafe-list" v-loading="loading">
      ...
      <div class="list-box" :class="`animation-${index+1}`" v-for="(item,index) in openCafeList" :key="item.id">
        <div class="cafe-box" @click="chooseCafe(item)">
          <img :src="item.url" />
          <div class="text">
            ...
            <p class="ellipsis--l2">
              ...
              <span v-if="item.orderway == 1" :title="item.sendtime ? sendTimeFormat(item.sendtime) : ''">
                  配送时间：
                  {{ item.sendtime ? sendTimeFormat(item.sendtime) : '--' }}
                  </span>
                  <span v-else :title="common.pickupTimeText(item)">
                    取餐时间：
                    {{common.pickupTimeText(item)}}
                  </span>
            </p>
          </div>
        </div>
...
<script>
import { fetchCafeListByType, fetchCafeConfigList } from "@/api/admin/cafe";
...
export default {
    ...
    openCafeList() {
      let list = this.cafeList.filter(i => {
        ...
        if (
          !i.openobj ||
          !i.opentime ||
          (!i.sendtime && i.orderway == 1) ||
          ((!i.ordertype || !i.preparetime) && i.orderway == 1) ||
          !i.pickuptime && (i.orderway == 2 || i.orderway == 3)
        ) {
          return false;
        } else {
          // 开放对象是否包含当前用户
  ...
  methods: {
    ...
    getCafeList() {
      this.loading = true;
      let param = {
        shopname: this.keyword || null,
        shoptypeid: this.activeCafeType == "all" ? null : this.activeCafeType
      };
      this.cafeList = [];
      fetchCafeListByType({
        filter: {
    ...
    initParams() {
      ...
      if (params) {
        ...
        this.keyword = data.shopname || null;
        this.activeCafeType = data.shoptypeid;
      }
    }
  },
  created() {
