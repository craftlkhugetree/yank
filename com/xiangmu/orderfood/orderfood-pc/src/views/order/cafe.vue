    ...
    <div class="base-search-table">
      ...
      <div v-loading="loading" class="detail">
        <p>
          <template v-if="shopDetail.orderway == 1">
            ...
            <span class="ellipsis" style="width: 300px">
              {{ shopDetail.preparetime || '--' }}{{ common.unitFormat(shopDetail.prepareunit) }}
            </span>
          </template>
          <template v-else>
            <label>取餐时间：</label>
            <el-tooltip :content="common.pickupTimeText(shopDetail)" placement="top-start">
              <span class="ellipsis" style="width: 300px">
                {{ common.pickupTimeText(shopDetail) }}
              </span>
            </el-tooltip>
          </template>
          <label>联系电话：</label>
        ...
        <p>
          ...
          <el-tooltip
            v-if="shopDetail.orderway == 1"
            :content="sendTimeFormat(shopDetail.sendtime)"
            placement="top-start"
          >
            <span class="ellipsis" style="width: 300px">
              {{ sendTimeFormat(shopDetail.sendtime) }}
            </span>
          </el-tooltip>
          <template v-if="shopDetail.orderway == 3">
            <label>预定时间：</label>
            <el-tooltip
              :content="
                shopDetail.fixedtime1 && shopDetail.fixedtime2
                  ? shopDetail.fixedtime1 + ' ~ ' + shopDetail.fixedtime2
                  : '--'
              "
              placement="top-start"
    ...
    <div class="food-list" v-loading="typeLoading">
      ...
      <div class="tab-content clearfix" v-loading="foodLoading">
        <div class="search-box-right">
          ...
          ></el-input>
          <el-button type="primary" size="small" icon="el-icon-search" @click="getFoodList">
            查询
          </el-button>
        </div>
        <div
        ...
        >
          ...
          ></el-input-number>
          <span
            v-if="food.num == 0 && (food.lastnum > 0 || food.lastnum == null)"
            class="btn"
          ...
          <span
            v-if="food.num == 0 && food.lastnum <= 0"
            class="btn disabled"
...
import { fetchTypeList, fetchAllFoodList, fetchTypeNums } from '@/api/admin/food';
export default {
  props: {
    shopid: String,
    shopname: String,
  ...
  methods: {
    ...
    initShopCart() {
      ...
      this.curShopCart = this.shopCartList.filter(i => i.shopid == this.shopid)[0] || null;
      if (this.curShopCart) {
        // 获取该餐厅下的所有菜品
        fetchAllFoodList({
          shopid: this.shopid,
...
.food-box {
  ...
  padding-bottom: 10px;
  background: #ffffff;
  box-shadow: 0px 5px 8px 4px rgba(110, 115, 132, 0.05), 0px 3px 6px 0px rgba(110, 115, 132, 0.08),
