      ...
      >
        ...
        <el-table-column prop="orderstatus" label="费用结算" show-overflow-tooltip width="100">
          <template slot-scope="scope">
            <span :class="scope.row.status == '2' ? 'blue-tag' : 'orange-tag'">
              {{ scope.row.status == '2' ? '已转账' : '待转账' }}
            </span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column label="操作" align="center" width="260">
          <template slot-scope="scope">
            ...
            <span @click="toPrint(scope.row)">下载转账凭证</span>
          </template>
        </el-table-column>
      </el-table>
      <!---------------------------- 分页 ---------------------------->
      ...
      fetchSettleList(param)
        .then(res => {
          ...
          if (res.success) {
            ...
            sessionStorage.setItem('settledParams', JSON.stringify(settledParams));
          }
        })
        .catch(err => {
          this.tableLoading = false;
