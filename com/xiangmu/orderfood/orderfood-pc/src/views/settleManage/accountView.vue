<template>
  <div class="base-search-table" v-loading="loading" v-if="settleDetail.id">
    ...
    <div class="search-box clearfix" style="padding: 15px 20px 10px">
      <i class="el-icon-arrow-left" @click="reback">返回</i>
      ...
      >
        {{ settleDetail.status == '2' ? '已转账' : '待转账' }}
      </span>
      <div class="search-box-right">
        <i class="el-icon-printer" @click="toPrint">下载转账凭证</i>
        <i class="el-icon-circle-check" v-if="settleDetail.status == '1'" @click="toConfirm">
          确认转账
        </i>
      </div>
    </div>
    ...
    <div class="settleDetail">
      <div class="item">
        ...
        <span style="margin-left: 84px">
          结算时间：
          {{ moment(settleDetail.settletime, 'YYYYMMDDhhmmss').format('YYYY-MM-DD HH:mm:ss') }}
        </span>
      </div>
      <div class="item">
        <label>转账凭证</label>
    ...
    <el-tabs v-model="activeTab" @tab-click="handleClick">
      <el-tab-pane v-for="tab in tabs" :key="tab.id" :name="tab.id">
        <span slot="label" style="min-width: 50px">订单 {{ tab.tid }}</span>
...
<script>
import orderDetail from '../settle/components/orderDetail';
import { fetchSettleDetail, confirmSettle } from '@/api/admin/settle';
export default {
  components: {
    orderDetail,
  },
  ...
  methods: {
    reback() {
      let activeTab = this.settleDetail.status == '2' ? '3' : '2';
      this.$router.push(`/checkout?curActiveTab=${activeTab}`);
    },
    // 打印
    toPrint() {
      this.$confirm(`请打印内部转账凭证后，签字送到餐厅第一窗口，或联系餐厅管理员！`, '提示', {
