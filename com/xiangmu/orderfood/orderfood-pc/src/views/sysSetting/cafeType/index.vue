  ...
  <div class="base-search-table">
    ...
    <div class="table-content">
      ...
      >
        ...
        <el-table-column prop="name" label="餐厅类别" show-overflow-tooltip min-width="240">
...
export default {
  ...
  data() {
    return {
      loading: false,
      uploadUrl: window.g.url + "shoptype/handleTemplate",
      keyword: null,
      tableData: [],
      total: 0,
      currentPage: 1,
  ...
  methods: {
    // 下载模板
    downloadTemp() {
      let url = `${window.g.url}shoptype/downloadTemplate`;
    ...
    changeStatus(row) {
      ...
      let msg = `是否确认${message}该类别？`;
      let shops = row.shops || [];
      if (message == "不启用" && shops.length > 0) {
        msg = `已存在餐厅设置类别『 ${row.name} 』，不启用该类别将同时删除对应餐厅的餐厅类别，是否确认不启用？`
      }
      this.$confirm(msg, "提示", {
    ...
    // 删除
    deleteRow(row) {
      let msg = `是否确认删除类别『 ${row.name} 』？`;
      let shops = row.shops || [];
      if (shops.length > 0) {
        msg = `已存在餐厅设置类别『 ${row.name} 』，删除类别将同时删除对应餐厅的餐厅类别，是否确认删除？`
      }
      this.$confirm(msg, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
    ...
    // 排序
    sort(type, row) {
      let func = type == "up" ? sortCafeTypeUp : sortCafeTypeDown;
      this.loading = true;
      func({
        id: row.id
      })
