            ...
            <div v-else>
              ...
              <div class="btns">
                ...
                <div class="clearfix" style="float: right; width: 44%">
                  <i class="el-icon-closew" style="width: 50%" @click.stop="saveType(item, index)">
                    确定
                  </i>
                  <i
          ...
          >
            ...
            <el-table-column label="操作" align="center" width="240">
              <template slot-scope="scope">
                ...
                <span @click="changeRow(scope.row)">
                  {{ activeTab == '1' ? '下架' : '恢复上架' }}
                </span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    batchOp() {
      ...
      let param = {
        ids: this.multipleSelection.map(m => m.id).join(','),
        status,
      };
      this.tableLoading = true;
      changeStatusBatch(param)
        .then(res => {
          ...
          if (res.success) {
            ...
            this.$message({
              showClose: true,
              type: 'error',
              message: `批量${msg}失败：${res.data.message}`,
            });
          }
        })
        .catch(err => {
