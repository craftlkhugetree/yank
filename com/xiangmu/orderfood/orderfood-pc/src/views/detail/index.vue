    ...
    <div class="detail">
      ...
      <div class="item" v-if="detail.ordertype == 'group'">
        ...
        <span
          ...
          v-if="detail.orderway != 3"
        >期望送达： <span :class="common.expectFetch(detail) == '--' ? '' : 'orange-font'">{{common.expectFetch(detail)}}</span></span>
        <br v-if="detail.orderway != 3"/>
      ...
      <div class="item" v-else>
        ...
        <span
          style="margin-left:84px;"
          v-if="detail.orderway != 3"
        >期望取餐： <span :class="common.expectFetch(detail) == '--' ? '' : 'orange-font'">{{common.expectFetch(detail)}}</span></span>
        <br v-if="detail.orderway != 3"/>
        <span style="margin-left:84px;" v-if="detail.flow">取餐序号： <span class="orange-font">#{{detail.flow}}</span></span>
      </div>
      <div class="item">
        <label>订单信息</label>
...
export default {
  ...
  methods: {
      ...
      fetchOrderDetail(param)
        .then(res => {
          ...
          if (res.success) {
            ...
            this.setLastpaytime();
            this.detail.time = `${this.detail.senddate ? this.moment(
    ...
    openPayDialog() {
      let msg = '';
      if (this.isOrderOne) {
        msg = `您的订餐已成功提交！请于${this.detail.senddate ? this.moment(
          this.detail.senddate,"YYYYMMDD").format("YYYY-MM-DD") : '规定日期'} ${this.detail.sendtime || ''}前到${this.detail.shopname}取餐~`;
      } else {
        msg = `您的订餐已成功提交！取餐日期内${this.common.pickupTimeText(this.shopDetail)}，地点：${this.detail.shopname}~`
      }
