  ...
  <div>
        ...
        <div class="card">
          <div class="title-box">
            <p class="shop-title" @click="openShowTips(tab)">
              <span v-if="tab.orderway == 3" class="ellipsis">
                预定时间：{{
                  tab.fixedtime1 && tab.fixedtime2 ? tab.fixedtime1 + ' ~ ' + tab.fixedtime2 : '--'
                }}
              </span>
              ...
              <span class="ellipsis" v-if="tab.orderway != 3">
                餐厅说明：{{ tab.note || '--' }}
              </span>
            </p>
    ...
    <van-popup v-model="showTips" round position="bottom" :style="{ height: '50%' }">
      <div class="shopTips">
        ...
        <div v-if="curshop.orderway == 3">
          <label class="info-label">预定时间</label>
          <p class="gray-info">
            {{ curshop.fixedtime1 && curshop.fixedtime2 ? curshop.fixedtime1 + ' ~ ' + curshop.fixedtime2 : '--' }}
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    openShowPhone(shop) {
      this.phoneList =
        shop.mobile && shop.mobile.includes(',') ? shop.mobile.split(',') : [shop.mobile || ''];
      this.showPhone = true;
    },
    //显示餐厅说明
    openShowTips(shop) {
