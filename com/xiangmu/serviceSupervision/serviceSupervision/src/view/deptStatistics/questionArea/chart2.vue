<template>
  <div>
    ...
    <p class="title title2">
      问题分类提问量排行<span class="changebar">
    ...
    <div class="wbgc">
      ...
      <p class="tabletitle">类型提问数量排行</p>
      <div class="chart" ref="chart"></div>
      <p class="tabletitle">类型提问数量详情</p>
      <div class="datatable">
...
export default {
  ...
  created() {
    this.activeType = this.$store.state.userInfo.userOrgId[0].ID;
  },
  methods: {
    changeTimeType() {
      this.selectValue = new Date();
    ...
    exportData() {
      ...
      window.open(window.g.url + "/dataArea/exportQuesttype?" + str.join("&"));
    },
    clicktab() {
      this.getData(this.selectValue);
    },
