        ...
        <el-tab-pane label="用户" name="user">
          ...
          <div v-else>
            ...
            >
              ...
              <el-table-column prop="res" label="业务领域" show-overflow-tooltip>
                <template slot-scope="scope">
                  <el-select
...
export default {
  ...
  methods: {
    ...
    // 移除单个标签
    removeTag(val, row) {
      const obj = row && row.groupBK && row.groupBK.find(g => g.ID === val.ID) || {}
        ...
        this.authList.forEach(i => {
          ...
          arr.forEach(k => {
            if (k.RESID) {
              ...
              // 如果勾选了分析维度
              if(k.ID == "9100003-3-2") {
                checkList = checkList.concat(["9100003-4","9100003-5","9100003-6","9100003-7","9100003-8"]);
              // 如果勾选了业务领域分析
              } else if(k.ID == "9100003-3-3") {
                checkList = checkList.concat(["9100003-9","9100003-a","9100003-b","9100003-c","9100003-d","9100003-e"]);
              }
            }
          })
        });
