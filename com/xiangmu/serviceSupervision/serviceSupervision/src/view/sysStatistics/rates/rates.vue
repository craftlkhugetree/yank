      ...
      <div class="count-box-content clearfix">
        <div class="num-box" style="width: 20%">
          ...
          <p>
            ...
            <span style="margin-left:6px;">提问量 {{totalData['all-2']}}</span>
          </p>
        </div>
        <div class="num-box" style="width: 16%">
          <label>一星评价</label>
          <p>
            <countTo class="counttospan" :startVal="0" :endVal="totalData['all1']" :duration="1000"></countTo>
            <span style="margin-left:6px;">今日 {{totalData['today1']}}</span>
          </p>
        </div>
        <div class="num-box" style="width: 16%">
          <label>二星评价</label>
          <p>
            <countTo class="counttospan" :startVal="0" :endVal="totalData['all2']" :duration="1000"></countTo>
            <span style="margin-left:6px;">今日 {{totalData['today2']}}</span>
          </p>
        </div>
        <div class="num-box" style="width: 16%">
          <label>三星评价</label>
          <p>
            <countTo class="counttospan" :startVal="0" :endVal="totalData['all3']" :duration="1000"></countTo>
            <span style="margin-left:6px;">今日 {{totalData['today3']}}</span>
          </p>
        </div>
        <div class="num-box" style="width: 16%">
          <label>四星评价</label>
          <p>
            <countTo class="counttospan" :startVal="0" :endVal="totalData['all4']" :duration="1000"></countTo>
            <span style="margin-left:6px;">今日 {{totalData['today4']}}</span>
          </p>
        </div>
        <div class="num-box" style="width: 16%">
          <label>五星评价</label>
          <p>
            <countTo class="counttospan" :startVal="0" :endVal="totalData['all5']" :duration="1000"></countTo>
            <span style="margin-left:6px;">今日 {{totalData['today5']}}</span>
          </p>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 导出趋势数据
    exportData() {
      let url = `${window.g.url}dataMark/exportMarkAllBottom`;
      let handledeptid = this.activeTab == "all" ? "" : this.activeTab;
      let query = `?timetype=${this.timeType}&time=${this.time}&handledeptid=${handledeptid}`;
    ...
    exportDeptData() {
      ...
      let handledeptid = this.activeDeptTab == "all" ? "" : this.activeDeptTab;
      let query = `?starttime=${starttime}&endtime=${endtime}&handledeptid=${handledeptid}`;
      window.open(url + query, "_blank");
    },
    // 初始化图表
    initCharts() {
      ...
      this.util
        ...
        .then(res => {
          ...
          if (res.success) {
            let data = res.data || [];
            data.forEach(i => {
              this.totalData[i.timetype + i.markscore] = i.num;
            });
          }
        })
        .catch(err => {
    ...
    getDeptData() {
      ...
      if (this.activeDeptTab !== "all") {
        param.handledeptid = this.activeDeptTab;
      }
      this.util
        .postAjax({
          code: this.global.code,
