<template>
  <div>
    ...
    <div class="tabs" ref="tableTabs">
      ...
      >{{item.text}}</li>
      <!-- 新增记录 -->
      <van-button size="small" icon="plus" round type="primary" @click="addRecord">新增记录</van-button>
    </div>
    ...
    <div class="form-btns more-btns" ref="btns">
      ...
      <van-button v-if="resDetail.resstatus =='2'" type="default" @click="changeStatus('1')">开启</van-button>
      <van-button v-if="resDetail.resstatus =='1' && resDetail.usestatus == '1'" type="default" @click="changeStatus('2')">关闭</van-button>
      <van-button v-if="resDetail.usestatus == '1'" type="default" @click="deleteRes">删除</van-button>
    ...
    >
      <van-form ref="editForm">
        ...
        <van-field
          ...
          placeholder="请输入"
          type="number"
          :rules="[{ validator:(v) => {return v >= lastShowv},  message: '请填写大于上一次记录的数值' }]"
        />
      </van-form>
    </van-dialog>
    <!-- 是否确定删除 -->
...
export default {
  ...
  methods: {
    ...
    addRecord() {
      this.dialogTitle =
        this.activeTableTab === "1" ? "新增水表记录" : "新增电表记录";
    ...
    editRecord(row) {
      this.dialogTitle =
        this.activeTableTab === "1" ? "水表记录编辑" : "电表记录编辑";
      this.showDialog = true;
      this.curRow = row;
      ...
      if (action === "cancel") {
        ...
        this.$refs.editForm
          ...
          .then(() => {
            ...
            let url =
              this.doType === "add" ? "/spreselec/add" : "/spreselec/update";
            this.$toast.loading({
              message: "提交中...",
              forbidClick: true,
              duration: 0
    ...
    toEdit() {
      this.$router.push(
        `/edures-bm/res-info-manage/${this.restypeid}/edit-res/${this.id}`
      );
    },
    // 开启、关闭
    changeStatus(type) {
