    ...
    <el-table :data="tableData" style="width: 100%;" :height="tableHeight" :border="false">
      ...
      <el-table-column label="操作" fixed="right" width="110" align="center">
        <template slot-scope="scope">
          ...
          <span style="color:#faac16;padding:0 5px;font-weight:bold" @click="toEdit(scope.row)">编辑</span>
          <span
...
export default {
  ...
  methods: {
    ...
    toEdit(row) {
      this.$router.push(
        `/edures-bm/res-info-manage/${this.id}/edit-res/${row.id}`
      );
    },
    // 详情页面
    toDetail(row) {
      this.$router.push(
        `/edures-bm/res-info-manage/${this.id}/detail-res/${row.id}`
      );
    },
    // 开启、关闭
      ...
      this.util
        ...
        .then(res => {
          if (res.success) {
            ...
            this.typeDetail.chargecycle = this.common.chargecycleFormatter(
