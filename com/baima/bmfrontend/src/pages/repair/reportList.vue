<template>
  <div class="my">
    ...
    >
      <!--    盒子动画效果-->
      <transition name="fade" v-if="!item.isExpand" :duration="1000">
      ...
      <transition name="fadeDetail" v-else :duration="1000">
        <!------------------------- 详情 ------------------------->
        <detail
          ref="detail"
          :activeTab="activeTab"
        ...
        ></detail>
        <!-- <detail ref="detail" v-show="item.isExpand" :activeTab="activeTab"></detail> -->
      </transition>
    </div>
  ...
  methods: {
    ...
    toDetail(row) {
      ...
      this.$nextTick(() => {
        timer = setInterval(() => {
          if (this.$refs.detail) {
            this.$refs.detail[0].findById(row.id);
...
.fadeDetail-leave-from,   // 离开前,进入后透明度是1
.fadeDetail-enter-to {
  opacity: 1;
}
