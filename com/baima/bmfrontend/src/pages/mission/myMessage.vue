...
import { bizNode } from "@/assets/js/options";
...
export default {
  ...
  computed: {
    role: () =>
      sessionStorage.getItem("url4bizNode") &&
      JSON.parse(sessionStorage.getItem("url4bizNode"))
  },
  methods: {
    changeTab() {
      this.currentPage = 1;
  ...
  created() {
    // for (let str in bizNode) {
    //   if (this.role[str]) {
    //     this.biz = bizNode[str];
    //     break;
    //   }
    // }
    this.getList();
