<template>
    <div class="common-content">
      ...
      <div class="search-group">
        <!--<span>
          <label>学院</label>
          <el-select v-model="searchForm.orgid" placeholder="请选择" size="small" style="width: 150px">
...
<!--        <el-table-column prop="name" label="申请编号" width="180"></el-table-column>-->
        ...
        <el-table-column prop="actionstatus" label="状态" align="center" :formatter="common.applyLiveStatusFormatter"></el-table-column>
        <el-table-column label="操作" fixed="right" width="150" align="center">
          <template slot-scope="scope">
            <div class="table-btn green" @click="goDetail(scope.row)">详情</div>
<!--            <div v-if="(scope.row.usetype == '1' || scope.row.usetype == '2') && scope.row.actionstatus=='0' && scope.row.applystatus ==3" class="table-btn purple" @click="goDetail(scope.row)">入驻</div>-->
            <div v-if="scope.row.applystatus == 1" class="table-btn purple" @click="goAudit(scope.row)">审批</div>
<!--            <div v-if="(scope.row.usetype == '3') && scope.row.actionstatus=='1' && scope.row.applystatus ==3 " class="table-btn pink" @click="reCall(scope.row)">退出</div>-->
          </template>
        </el-table-column>
      </el-table>
  ...
  // import applyForm from "../../components/irrDialog"
    export default {
      ...
      methods:{
          ...
          goDetail(row){
            this.$router.push({
              path:`resource-leader-audit/detail/${row.id}`,
              query:{
          ...
          goAudit(row){
            this.$router.push({
              path:`resource-leader-audit/audit/${row.id}`,
              query:{
                activeName:this.activeName,
                currentPage:this.currentPage
              }
          ...
          getList(page){
            ...
            this.util.postAjax({
              ...
              if(res.success == true){
                // console.log(res);
                 this.applyList = res.item.leaderList.list;
