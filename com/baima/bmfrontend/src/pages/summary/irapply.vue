  ...
  <div class="common-content">
    ...
    <div class="search-group">
     <!-- <span>
      ...
      <span>
          <label>灌溉类型</label>
          <el-select v-model="searchForm.irrestypeid" placeholder="请选择" size="small" style="width: 150px">
    ...
    <el-table class="my-table" :data="resList" style="width: 100%" stripe ref="multipleTable" v-loading="loading">
      ...
      <el-table-column prop="classleadername" label="负责人" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column prop="typename" label="灌溉类型" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column prop="rescodes" label="资源编号" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column prop="irdate" label="灌溉日期" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          {{scope.row.irdate ? util.formatTime(scope.row.irdate, "yyyy.MM.dd") :null}}
        </template>
      </el-table-column>
      <el-table-column prop="applystatus" label="申请单状态" align="center">
        <template slot-scope="scope">
          <span :class="common.statusColor('','',scope.row.applystatus)">{{common.statusFormatter("","",scope.row.applystatus)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="note" label="备注"  width="300" show-overflow-tooltip></el-table-column>
    </el-table>
    ...
    methods:{
      ...
      download(){
        let params=JSON.stringify({
          irrestypeid:this.searchForm.irrestypeid,
          orgid:this.searchForm.orgid,
          rescode:this.searchForm.rescode,
          }
        )
        console.log(params);
        this.util.getUrlByCode(this.global.code, "/data/exportIrapply").then(res=>{
