<template>
  <div class="user-learn">
    <!---------------------------- 模块步骤条 ---------------------------->
    <model-step :list="examLearnData" :exam="curExam" @doFunc="getLoginUserRecords"></model-step>
    <div class="learn-content">
      <!---------------------------- 左侧内容 ---------------------------->
      <div class="content-left">
        <h3>{{detail.name}}</h3>
        <p class="sub">{{detail.watchCount}}阅读 · {{detail.downloadCount}}下载</p>
        <!---------------------------- 视频和文档 ---------------------------->
        ...
        <!---------------------------- 在线文章 ---------------------------->
        <div class="dhtml" v-if="detail.type == '3'" v-html="detail.dhtml"></div>
      </div>
      <!---------------------------- 右侧内容 ---------------------------->
      <div class="content-right">
        <div class="learn-list">
          ...
          <ul>
            ...
            >
              ...
              <span class="size" v-if="item.type != 3">{{common.learnSizeFormat(item.fileSize)}}</span>
        ...
        <div class="learn-time" v-loading="loading">
          ...
          >
            <span v-if="curExam.isPass == '1'">恭喜，考了{{curExam.score}}分，考试已通过</span>
            <span v-if="curModel.isPass == '1'">恭喜，考了{{curModel.score}}分，本关卡已通过</span>
          </p>
          <div v-else>
            <el-button
              class="orange-btn"
...
export default {
  ...
  methods: {
    ...
    toExam() {
      ...
      let content = exam.type == '2' ? exam.contents.find(i => i.modelId == this.curModel.id) : exam.contents[0]
    ...
    setLastNeedLearn() {
      ...
      if (this.curExam.type == '2') {
        let moduleRecords = this.records.filter(j => j.modelIds.includes(this.curModel.id))
        // 当前模块的学习时间
        this.nowLearnTime = this.curModel.learnTimes = moduleRecords.reduce((sum, cur) => {
          return sum + cur.duration * 60
        }, 0)
      ...
      this.nowTimer = setInterval(() => {
        this.nowLearnTime += 1
        this.lastNeedLearn = this.lastNeedLearn > 0 ? this.lastNeedLearn - 1 : 0
      }, 1000)
    }
  },
  beforeDestroy() {
