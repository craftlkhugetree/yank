  ...
  <div class="base-search-table detail">
    <div class="search-box clearfix">
      ...
      <div class="search-box-right" style="max-width: 100%">
        ...
        <el-button size="small" class="bule-border" @click="deleteTrain">删除</el-button>
      </div>
    </div>
    <div class="content">
      <!---------------------------- 标题 ---------------------------->
      ...
      <p
        class="sub"
      >{{detail.watchCount}}阅读 · {{detail.downloadCount}}下载 · {{common.learnSizeFormat(detail.fileSize)}} · {{common.defaultTimeFormat(detail.createTime)}}</p>
      <!---------------------------- 视频和文档 ---------------------------->
      ...
      <div class="tag-box">
        <div>
          <label>所属模块：</label>
          <div class="tags">
            <span v-for="item in detail.models" :key="item.id">{{item.name}}</span>
          </div>
        </div>
        <div>
          <label>所属校区：</label>
          <div class="tags">
            <span v-for="item in detail.campuss" :key="item.id">{{item.name}}</span>
          </div>
        </div>
        <div>
          <label>用户类型：</label>
          <div class="tags">
            <span v-for="item in detail.usertypes" :key="item.id">{{item.name}}</span>
          </div>
        </div>
        <div>
          <label>开启下载：</label>
...
export default {
  ...
  methods: {
    ...
    // 删除
    deleteTrain() {
      this.$confirm(`是否确认删除培训资料『 ${this.detail.name} 』？`, '提示', {
