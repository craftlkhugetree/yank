  ...
  <div class="base-search-table" v-loading="loading">
    ...
    <div class="search-box clearfix">
      ...
      <div class="search-box-right" style="max-width: 100%">
        <el-button type="primary" size="small" @click="save" v-loading="loading"
          >保存</el-button
        >
      </div>
    </div>
    ...
    <div class="form-box">
      ...
      >
        ...
        <el-form-item label="适用校区" prop="campusIds">
          <el-checkbox-group
            v-model="editForm.campusIds"
            @change="debouncePoolData"
          >
            <el-checkbox
        ...
        <el-form-item label="适用用户" prop="usertypeCodes">
          <el-checkbox-group
            v-model="editForm.usertypeCodes"
            @change="debouncePoolData"
          >
            <el-checkbox
        ...
        <el-row :gutter="6" v-loading="questionLoading">
          ...
          <el-col :span="8">
            ...
            >
              <question-tips
                :multi="editForm.campusIds.length * editForm.usertypeCodes.length"
                ref="judgePool"
                :data="poolData"
                type="3"
              ></question-tips>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        name: [
          { required: true, trigger: "change", message: "请输入考试名称" },
        ],
        examDate: [
          { required: true, trigger: "change", message: "请选择考试时间" },
        ],
        isOpenip: [
          { required: true, trigger: "change", message: "请选择IP限制" },
        ],
        ipStart: [{ required: true, validator: validateIp, trigger: "change" }],
        ipEnd: [{ required: true, validator: validateIp, trigger: "change" }],
        campusIds: [
          { required: true, trigger: "change", message: "请选择适用校区" },
        ],
        usertypeCodes: [
          { required: true, trigger: "change", message: "请选择适用用户" },
        ],
        modelIds: [
          { required: true, trigger: "change", message: "请选择题目模块" },
        ],
      },
      detail: {},
      poolData: [],
  ...
  computed: mapState({
    ...
    multi: () => {
      if (this && this.editForm) {
        return this.editForm.campusIds.length * this.editForm.usertypeCodes.length
      }
    }
  }),
  methods: {
    ...
    save() {
      this.$refs.editForm.validate((valid) => {
        if (valid) {
          ...
          if (!(param.content.judgeNum + param.content.multipleNum + param.content.singleNum)) {
            this.$message({
              type: 'warning',
              message: "没有设置题目！",
              showClose: true
    ...
    // 题目总数动态变化
    dyQueNum(old, newMax) {
      if (this.editForm.content[old] > newMax) {
        this.$set(this.editForm.content, old, newMax);
      }
    },
    // 题目池数量
    getPoolData() {
        ...
        .then((res) => {
          if (res.code == "000000") {
            ...
            this.$nextTick(() => {
              ...
              this.dyQueNum("judgeNum", this.judgePoolMax);
              this.dyQueNum("singleNum", this.singlePoolMax);
              this.dyQueNum("multipleNum", this.multiplePoolMax);
            });
    ...
    initDetail(data) {
      ...
      this.editForm.campusIds = data.campuss
        ? data.campuss.map((i) => i.id)
        : [];
      this.editForm.usertypeCodes = data.usertypes
        ? data.usertypes.map((i) => i.code)
        : [];
