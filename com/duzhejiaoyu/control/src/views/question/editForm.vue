    ...
    <div class="form-box">
      ...
      >
        <el-form-item label="题目形式" prop="type">
          <el-radio-group v-model="editForm.type" :disabled="isEdit" @change="changeType">
            <el-radio
        ...
        >
          <div class="upload-box">
            <p class="img-des">
              建议尺寸为1000*300，大小不超过10M，支持格式 png、jpg、gif
            </p>
            <img-upload
              ref="imgUpload"
              :url="imgUploadUrl"
        ...
        <el-form-item
          ...
          :rules="[
            {
              required: ['1', '2'].includes(editForm.type),
              trigger: 'change',
        ...
        <el-form-item label="是否必考" prop="isImportant">
          <div class="switch">
            ...
            <span
              class="on"
              v-show="editForm.isImportant == 1"
              @click="editForm.isImportant = 0"
              >是</span
            >
            <span
              class="off"
              v-show="editForm.isImportant == 0"
              @click="editForm.isImportant = 1"
              >否</span
            >
          </div>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        type: [
          { required: true, trigger: "change", message: "请选择题目形式" },
        ],
        subject: [
          { required: true, trigger: "change", message: "请输入题目内容" },
        ],
      },
      detail: {},
      optionIndex: null,
  ...
  methods: {
    changeType(newVal) {
      this.all[this.oldVal - 1] = _.cloneDeep(this.editForm);
      this.all[this.oldVal - 1].type = this.oldVal;
      this.$store.commit("setQType", newVal + "");
      this.editForm = this.all[newVal - 1];
      this.oldVal = this.editForm.type;
    },
    // 校验题目选项
              ...
              if (res.code == "000000") {
                this.$confirm("您的题目已保存成功！", "保存成功", {
                  ...
                  .catch(() => {
                    ...
                    this.editForm.options.splice(0, 0, { itemInfo: null, isReal: 1 });
                    this.editForm.options.splice(1, 0, { itemInfo: null, isReal: 0 });
                    this.optionIndex = null;
                    this.optionIndexList = [];
                    this.editForm.type = this.$store.state.qType;
                  });
              } else {
                this.$message({
                  showClose: true,
    ...
    initDetail(data) {
      ...
      if (data.type == "2") {
        this.optionIndexList = data.options
          .filter((i) => i.isReal == 1)
          .map((i, index) => index);
      }
