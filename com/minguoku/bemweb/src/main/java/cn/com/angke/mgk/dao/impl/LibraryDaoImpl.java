...
@Service
public class LibraryDaoImpl extends PlatformBaseDaoImpl implements LibraryDao {

	public String TABLE_NAME = "WXK_LIBRARY";

	public String SQL_SELECT = "select ID, NAME, OPAC "
			+ "from WXK_LIBRARY a";
	// tree
	...
	public String SQL_COUNT = "select count(a.ID) from WXK_LIBRARY a \n";
		...
		for (int i = 0; i < po.getConditions().size(); i++) {
			...
			if (!StringUtils.isBlank(value)) {
				...
				if (field.equals("ID")) {
					...
					sqlParas.addPara("%"+value+"%", Types.VARCHAR);
					whereStr.append(" a.OPAC like ? and ");
					sqlParas.addPara("%"+value+"%", Types.VARCHAR);
				}
			}
		}
