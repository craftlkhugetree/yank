...
@Service
public class ZtTypeDaoImpl extends PlatformBaseDaoImpl implements ZtTypeDao {

	public String TABLE_NAME = "WXK_ZTTYPE";

	public String SQL_SELECT = "select ID, PID, ZTNAME, ZTSYMBOL " + "from WXK_ZTTYPE a";
	// tree
	public String SQL_TREE = "select ID, PID, ZTNAME, ZTSYMBOL" + "from WXK_ZTTYPE a";
	public String COLUNM_NAME = "ID";

	public String SQL_VIEW = "";

	public String SQL_COUNT = "select count(a.ID) from WXK_ZTTYPE a \n";

	public ZtTypeDaoImpl() {
		super();
		...
		for (int i = 0; i < po.getConditions().size(); i++) {
			...
			if (!StringUtils.isBlank(value)) {
				...
				if (field.equals("ID")) {
					...
					whereStr.append(" a.ZTNAME like ? and ");
					sqlParas.addPara("%" + value + "%", Types.VARCHAR);
				} else if (field.equals("ZTTYPE")) {
						whereStr.append(" a.ZTSYMBOL = ? and ");
						sqlParas.addPara(value, Types.VARCHAR);
					} 
			}
		}
	...
	@Override
	public JSONArray getZtSymbol() {
		String sql = "select ZTSYMBOL from WXK_ZTTYPE";
		List<JSONObject> list = this.getJdbcTemplate().query(sql, readRows);
		JSONArray arr = new JSONArray();
		for(int i=0;i<list.size();i++){
			arr.add(list.get(i).getString("ZTSYMBOL"));
		}
		return arr;
	}
