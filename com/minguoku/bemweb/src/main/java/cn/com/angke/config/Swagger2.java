...
public class Swagger2 extends WebMvcConfigurationSupport {
	...
	public Docket createRestApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).pathProvider(new ProjectPath(this.getServletContext())).select()
				.apis(RequestHandlerSelectors.basePackage("cn.com.angke.app.controller")).paths(PathSelectors.any()).build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("民国库管理端 APIs").description("")
