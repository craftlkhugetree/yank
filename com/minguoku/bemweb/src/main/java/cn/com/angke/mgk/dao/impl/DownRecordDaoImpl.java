...
public class DownRecordDaoImpl extends PlatformBaseDaoImpl implements DownRecordDao {
	...
	public String SQL_VIEW = "select a.ID,a.BOOKID,b.TITLE,a.PAGENUM,a.USERIP,a.CTMID from WXK_DOWNRECORD a left join "
			+ "WXK_BOOK2 b on a.BOOKID=b.ID";

	public String SQL_COUNT = "select count(a.ID) from WXK_DOWNRECORD a left join WXK_BOOK b on a.BOOKID=b.ID \n";
		...
		for (int i = 0; i < po.getConditions().size(); i++) {
			...
			if (!StringUtils.isBlank(value)) {
				...
				if (field.equals("ID")) {
					...
					whereStr.append(" b.TITLE like ? or");
					sqlParas.addPara("%"+value+"%", Types.VARCHAR);
					whereStr.append(" a.USERIP like ? and");
					sqlParas.addPara("%"+value+"%", Types.VARCHAR);
