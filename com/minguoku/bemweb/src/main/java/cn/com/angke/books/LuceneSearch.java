...
public class LuceneSearch {
	 ...
	 */
	@SuppressWarnings("deprecation")
	public JSONArray search2(String keyWord, Occur logic) throws IOException {
		if ((keyWord == null) || (keyWord.equals("")))
			return null;
		else {
			JSONArray array = new JSONArray();
			...
			for (int i = 0; i < key.length; i++) {
				if (i == 0)
					query.add(new TermQuery(new Term("content", key[i])), SEARCH_LOGIC_AND);
				else
					query.add(new TermQuery(new Term("content", key[i])), logic);
			}
			this.rebuildSearch();
			ScoreDoc[] hits = searcher.search(query, MaxRows).scoreDocs;
			System.out.println(hits.length);
	...
	public JSONArray search(String keyWord, Occur logic) throws IOException, ParseException {
		if ((keyWord == null) || (keyWord.equals("")))
			...
			//记录的得分排序
			TopFieldCollector collector = TopFieldCollector.create(sort , 10  ,  false , false , false);  
			this.rebuildSearch();
	...
	public JSONArray search(String bookID, String keyWord, Occur logic) throws IOException {
		if ((bookID == null) || (bookID.equals("")) || (keyWord == null) || (keyWord.equals("")))
			...
			for (ScoreDoc doc : hits) {
				...
				jsonObject.put("PZJ", document.get("zj"));
				jsonObject.put("PAGE", Integer.parseInt(document.get("page")));
				jsonObject.put("TITLE", "第" + document.get("vpage") + "页");
