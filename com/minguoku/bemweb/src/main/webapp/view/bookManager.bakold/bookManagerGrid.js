...
var CenterGrid= new Ext.grid.GridPanel({
    ...
    cm: new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel ({singleSelect : true}), //改为ture为多选
        ...
        {header: '责任人', dataIndex: 'AUTHOR',width:120},
        {header: '出版地', dataIndex: 'ADDRESS',width:120},
        {header: '出版社', dataIndex: 'PUBLISHINGNAME',width:120},
        {header:'出版年',dataIndex:'DATE',width:130},
        {header:'页数',dataIndex:'PAGECOUNT',width:120},
        {header: '宽度', dataIndex: 'WIDTH',width:140},
        {header: '高度', dataIndex: 'HEIGHT',width:140},
        {header: '主题词', dataIndex: 'TAGS',width:140},
        {header: '中图法分类', dataIndex: 'ZTNAME',width:140},
        {header: '附注说明', dataIndex: 'TEXTNOTE',width:120},
        {header: '内容对应扫描目录', dataIndex: 'BOOKDIR',width:120},
        {header: '丛书名', dataIndex: 'SERIES',width:140},
        {header:'对应学科',dataIndex:'EDUTYPEID',width:120}
    ]),
    // viewConfig: {
    //     forceFit: true
    // },
    bbar: new Ext.PagingToolbar({  
        ...
        listeners:{
            beforechange:function(a,param){
                var start=param.start;
                var page=param.start/16+1;
                var xx={"start":start,"limit":16,"page":page,"filter":batchFilter};
                param.data=JSON.stringify(xx);
