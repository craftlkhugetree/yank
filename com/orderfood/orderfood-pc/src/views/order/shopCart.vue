<template>
  <div class="base-search-table" v-loading="loading">
    <div class="search-box clearfix" style="padding: 15px 20px 10px;">
      ...
      <el-divider direction="vertical"></el-divider>购物车
    ...
    <div class="detail">
      ...
      <div class="item" style="position:relative;">
        ...
        <span
          ...
          :key="type.id"
          @click="curOrderType = type.id;showTip = true;"
        >{{type.name}}</span>
        <div v-if="showTip" class="type-tips" :class="{'top': curOrderType == 'group'}">
          <i class="el-icon-warning"></i>
          {{curOrderType == "group" ? "送餐上门后请扫码收餐，并及时结算订单。" : "需自行去往餐厅取餐。"}}
          <i
            class="el-icon-close"
            @click="showTip=false;"
          ></i>
      ...
      <div class="item" v-if="curOrderType == 'group'">
        ...
        >
          <span slot="reference" class="btn" @click="addressVisible = !addressVisible">选择地址</span>
            ...
            <el-radio-group v-model="curAddressId">
              <el-radio v-for="item in addressList" :key="item.id" :label="item.id">
                <span class="area">
                  ...
                  <i
                    class="el-icon-edit"
                    @click.stop="openDrawer('edit',item)"
                  ></i>
                  <i class="el-icon-delete" @click.stop="deleteAddress(item)"></i>
                </span>
                <span class="area-note">{{item.username}}（{{item.userid}}） {{item.mobile}}</span>
              </el-radio>
            </el-radio-group>
          </div>
        </el-popover>
      ...
      <!-- 预计取餐时间/日期 -->
      <div class="item">
        <h3 v-if="curOrderType == 'self'">{{isOrderOne ? "预计取餐时间" :
          shopDetail.orderway == 2 ? "预计取餐日期" : ""}}</h3>
        <h3 v-else>{{isOrderOne ? "期望送达时间" : 
          shopDetail.orderway == 2 ? "期望送达日期" : ""}}</h3>
          <div v-if="isOrderOne">
        <el-date-picker
          v-model="date"
          type="date"
...
<script>
import { saveOrder } from '@/api/admin/order'
...
export default {
  ...
  data() {
    return {
      ...
      pickerOptions: {
        disabledDate: time => {
          return time.getTime() < this.minTime - 24 * 60 * 60 * 1000 || time.getTime() > this.minTime + 6 * 24 * 60 * 60 * 1000
        }
      },
      pickupDate: null,
      pickerOptions2: {},
  ...
  methods: {
    focusPickupTime() {
      this.pickerOptions2 = {
         disabledDate: time => {
          ...
          let fixedtime1 = new Date(this.moment().format('YYYY-MM-DD') + ' ' + this.shopDetail.fixedtime1).getTime();
          let fixedtime2 = this.shopDetail.fixedtime2 && parseFloat(this.shopDetail.fixedtime2) || 0;
          if (now < fixedtime1) {
            return time.getTime() < now + 
          ((!fixedtime2 || fixedtime2 == '0') ?
           0 : (fixedtime2 - 1)) * 24 * 60 * 60 * 1000
          } else {
            return time.getTime() < now + 
          ((!fixedtime2 || fixedtime2 == '0') ?
           0 : (fixedtime2)) * 24 * 60 * 60 * 1000
          }
        }
      }
    },
    ...
    getAddressList() {
      fetchAddressList().then(res => {
        if (res.success) {
          ...
          if (!this.curAddressId && this.addressList.length > 0) {
    ...
    save() {
      ...
      if (
        ...
        (this.date == this.moment(this.minTime).format('YYYYMMDD') && this.time < this.minTime.format('HH:mm')))
      ) {
        this.$message({
          showClose: true,
          type: 'error',
    ...
    doSave() {
      this.loading = true
      let curAddress = this.curOrderType == 'group' ? this.addressList.filter(i => i.id == this.curAddressId)[0] : {}
      ...
      let senddate = this.isOrderOne ? this.date : this.shopDetail.orderway == 2 ? this.pickupDate : undefined;
        ...
        .then(res => {
          ...
          if (res.success) {
            ...
            if (this.curOrderType == 'group') {
              ...
              // 个人订餐需要支付
              let msg = `您的订餐已成功提交！请于${this.moment(this.date, 'YYYYMMDD').format('YYYY-MM-DD')} ${this.time}前到${
                this.shopCart.shopname
              }取餐~`
