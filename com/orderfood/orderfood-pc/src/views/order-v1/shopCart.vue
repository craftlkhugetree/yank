<template>
  <div class="base-search-table" v-loading="loading">
    <div class="search-box clearfix" style="padding: 15px 20px 10px;">
      <i class="el-icon-arrow-left" @click="$router.push('/order')">返回</i>
    ...
    <div class="detail">
      ...
      <div class="item">
        <h3>{{curOrderType == "self" ? "预计取餐时间" : "期望送达时间"}}</h3>
        <el-date-picker
          v-model="date"
          type="date"
          placeholder="选择日期"
...
export default {
  ...
  computed: {
    ...
    curAddressName() {
      ...
      return i
        ? `${i.area}${i.house}，${i.username}（${i.userid}），${i.mobile}`
        : null;
    },
    // 可以选择的菜品
    canSelectFoodList() {
  ...
  methods: {
    ...
    save() {
      ...
      if (!this.date || !this.time) {
        let msg =
          this.curOrderType == "group" ? "期望送达时间" : "预计取餐时间";
        this.$message({
          showClose: true,
          type: "error",
          message: `请选择${msg}！`
      ...
      if (
        this.curOrderType == "self" &&
        (!this.mobile || !/^1\d{10}$/.test(this.mobile))
      ) {
        this.$message({
          showClose: true,
          type: "error",
    ...
    doSave() {
      ...
      let mobile =
        this.curOrderType == "group" ? curAddress.mobile || null : this.mobile;
      let data = {
          ...
          if (res.success) {
            ...
            if (this.curOrderType == "group") {
              ...
              let msg = `您的订餐已成功提交！请于${this.moment(
                this.date,
                "YYYYMMDD"
              ).format("YYYY-MM-DD")} ${this.time}前到${
