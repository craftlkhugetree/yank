    ...
    <div class="detail">
      ...
      <div class="item">
        ...
        <div v-if="showTip" class="type-tips">
          ...
          {{detail.ordertype == "group" ? "送餐上门后请扫码收餐，并及时结算订单。" : "需自行去往餐厅取餐。"}}
          <i
            class="el-icon-close"
            @click="showTip=false;"
          ></i>
      ...
      <div class="item">
        <h3>{{detail.ordertype == "self" ? "预计取餐时间" : "期望送达时间"}}</h3>
        <el-date-picker
...
export default {
  ...
  methods: {
    ...
    getAddressList() {
      fetchAddressList().then(res => {
        if (res.success) {
          ...
          let curAddress = this.addressList.filter(
            i => i.title == this.detail.addressName
          )[0];
    ...
    save() {
      let msg =
        this.detail.ordertype == "group" ? "期望送达时间" : "预计取餐时间";
      if (!this.detail.senddate || !this.detail.sendtime) {
        this.$message({
          showClose: true,
          type: "error",
          message: `请选择${msg}！`
      ...
      if (
        ...
        (this.detail.senddate == this.moment(this.minTime).format('YYYYMMDD') && this.detail.sendtime < this.minTime.format('HH:mm'))
      ) {
        this.$message({
          showClose: true,
          type: "error",
      ...
      if (this.detail.senddate < this.moment().format("YYYYMMDD")) {
        this.$message({
          showClose: true,
          type: "error",
          message: "请重新选择时间！"
        ...
        if (res.success) {
          ...
          if (orderstatus == "1") {
              ...
              .then(res => {
                ...
                if (res.success) {
                  let msg =
                    obj.ordertype == "group"
                      ? "您的订餐已成功提交！"
                      : `您的订餐已成功提交！请于${this.moment(
                          obj.senddate,
                          "YYYYMMDD"
                        ).format("YYYY-MM-DD")} ${obj.sendtime}前到${
                          obj.shopname
                        }取餐~`;
            ...
            switch (orderstatus) {
              case "2":
                msg =
                  ordertype == "group" ? "订单正在配送中" : "订单正在等待取餐";
                break;
              case "3":
