  ...
  <div class="wrapper">
    ...
    <section class="main-wapper">
      <div class="address-box">
        ...
        >
          <template #title>
            <span class="ads-title"
              >{{ orderData.area }} {{ orderData.house }}</span
            >
          </template>
          <template #label>
            <span class="ads-des"
              >{{ orderData.creatername }}（{{ orderData.createrid }}）{{
                orderData.mobile
              }}</span
            >
          </template>
        ...
        <!---------------------------- 时间 -------------------------->
        <van-cell is-link @click="doChooseTime" class="info">
          <template #title>
            <span class="time-title">{{
              active == "group" ? "期望送达时间" : "期望取餐时间"
            }}</span>
          </template>
          <span class="time-day"
    ...
    >
      ...
      <p>
        ...
        <span class="blue-text">{{ cafeConfig.mobile }}</span> 联系餐厅确认~
      </p>
      <div class="bottom-btn">
        <van-button
          ...
          >取消</van-button
        >
        <van-button class="clear-btn" round type="info" @click="confirmSubmit"
          >确认提交</van-button
        >
      </div>
    </van-dialog>
            ...
            <div @click="chooseAds(ads)" class="address-info">
              <div class="icon-check">
                <van-icon
                  ...
                  v-if="
                    activeAds ==
                    `${ads.area}${ads.house}${ads.userid}${ads.mobile}`
                  "
                />
                <van-icon name="circle" class="budge" color="#DEDEDE" v-else />

                <span class="address-title"
                  >{{ ads.area }} {{ ads.house }}</span
                >
              </div>
      ...
      <div class="address">
        <van-form @submit="savePhone">
          <p class="head">
            ...
            <van-button round class="savebtn" type="info" native-type="submit"
              >保存</van-button
            >
          </p>
          <div class="info">
...
export default {
  ...
  methods: {
    ...
    initDayList() {
      let arr = [];
      for (let i = 0; i < 7; i++) {
        let next_day = this.moment(this.startDay)
          .add(i, "days")
          .format("YYYYMMDD");
        let weekArr = ["周日", "周一", "周二", "周三", "周四", "周五", "周六"];
        let nowWeek = this.moment(next_day).weekday(); //今天是周几
    ...
    onSubmit() {
      ...
      if (!this.defaultday || !this.activeTime) {
      ...
      if (
        ...
        (this.defaultday == this.moment(this.startDay).format("YYYYMMDD") && this.activeTime < this.moment().add(this.preparetime, "m").format("HH:mm"))
      ) {
      ...
      let sendDateTime = this.moment(
        this.defaultday + " " + this.activeTime,
        "YYYYMMDD HH:mm"
      );
      let now = this.moment();
    ...
    async handleBlur(event, food) {
      ...
      //个人限制--个人剩余量
      if (food.onelimit == "1") {
        let personSurplusNum =
          (await this.common.fetchItemBuyNum(food)) + food.initnum || 0;
    ...
    async addNum(food) {
      ...
      //个人限制--个人剩余量
      if (food.onelimit == "1") {
        let initIndex = this.initTableData.findIndex(
          (i) => i.itemid == food.itemid
        );
        if (initIndex > -1) {
          initvalue = this.initTableData[initIndex].initnum;
        }
        let personSurplusNum =
          (await this.common.fetchByitemid(food)) + initvalue || 0;
        if (food.itemnum >= personSurplusNum) {
          this.$toast.fail(`限${food.onelimitnum}份每人`);
          return false;
        }
      ...
      if (
        (food.alllimit == "1" &&
          food.itemnum > food.surplusnum + food.initnum) ||
        0
      ) {
    ...
    async handleMealBlur(event, food) {
      ...
      if (food.onelimit == "1") {
        ...
        let personSurplusNum =
          (await this.common.fetchItemBuyNum(food)) + initvalue;
    ...
    async addMeal(food) {
      ...
      if (food.onelimit == "1") {
        ...
        let personSurplusNum =
          (await this.common.fetchItemBuyNum(food)) + initvalue || 0;
        if (food.num >= personSurplusNum) {
          this.$toast.fail(`限${food.onelimitnum}份每人`);
          return false;
        }
    ...
    setSelectableTimeList() {
      ...
      this.activeTime = times.includes(this.activeTime)
        ? this.activeTime
        : times[0];
    },
  },
