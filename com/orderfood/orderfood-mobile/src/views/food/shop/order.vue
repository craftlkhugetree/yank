  ...
  <div class="wrapper">
      ...
      <div class="address-box">
        ...
        <div class="tips-box">
          ...
          <div class="yellow-tips" v-show="active == 'self' && showSelfTips">
            <van-icon name="warning" color="#FAAD14" size="20" />
            <span>需自行去往餐厅取餐。{{cafeConfig.orderway == 1 ? '' : common.pickupTimeText(cafeConfig)}}</span>
            <van-icon
              class="del-btn"
              name="cross"
              color="#8E8E8E"
        ...
        >
          ...
          <template #label>
            <div class="ads-des">
              ...
              {{ defaultAds && `（${defaultAds.userid}）`
              }}{{ defaultAds && defaultAds.mobile }}
            </div>
          </template>
        </van-cell>
        <!---------------------------- 个人订餐 -------------------------->
        ...
        <!---------------------------- 时间 -------------------------->
        <van-cell class="info" is-link @click="doChooseTime" v-if="[1,2].includes(cafeConfig.orderway)">
          <template #title>
        <span class="time-title" v-if="active == 'self'">{{isOrderOne ? "预计取餐时间" :
          cafeConfig.orderway == 2 ? "预计取餐日期" : ""}}</span>
        <span class="time-title" v-else>{{isOrderOne ? "期望送达时间" : 
          cafeConfig.orderway == 2 ? "期望送达日期" : ""}}</span>
          ...
          >
            <div class="address-info" @click="chooseAds(ads)">
              ...
              <p
                @click="chooseAds(ads)"
                class="gray-info"
              >{{ ads.username }}（{{ ads.userid }}）{{ ads.mobile }}</p>
            </div>
    ...
    <van-popup v-model="showPay" :close-on-click-overlay="false" round position="bottom">
      <div class="payinfo">
        ...
        <div
          class="tips"
        >校园卡支付有默认额度限制，教职工和研究生单笔200，当天400；本科生单笔30，当天200；超当天限额需先在校园卡-个人中心-消费设置中修改，或去充值机修改。</div>

        <van-button round class="paybtn" :loading="payLoading" type="info" @click="handlePay">确认支付</van-button>
      </div>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  computed: {
    minDate() {
      ...
      let fixedtime1 = new Date(this.moment().format('YYYY-MM-DD') + ' ' + this.cafeConfig.fixedtime1).getTime();
      ...
      if (now < fixedtime1) {
        ...
        return this.moment().add('days', fixedtime2 + 1).toDate()
      }
    },
    isOrderOne() {
  ...
  methods: {
    ...
    getAddress() {
      fetchAddress({}).then(res => {
        if (res.success) {
          ...
          this.defaultAds = this.addressArr.length ? this.addressArr[0] : null
    ...
    handleSubmit() {
      ...
      let senddate = this.isOrderOne ? this.defaultday : this.cafeConfig.orderway == 2 ? this.pickupTime : undefined;
