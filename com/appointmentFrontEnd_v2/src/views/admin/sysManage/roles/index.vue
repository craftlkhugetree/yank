<template>
  <div class="main-div" ref="mainDiv">
        ...
        <el-table-column label="操作" align="center" width="180">
          <template slot-scope="scope">
            <div v-if="!scope.row.isEdit">
              <span v-if="scope.row.CODE !=='default'" @click.stop="scope.row.isEdit = true">编辑</span>
    ...
    <div class="right-div base-search-table">
      <div class="search-box clearfix">
        ...
        <div class="search-box-right">
          ...
          >添加</el-button>
          <el-button type="danger" size="small" icon="el-icon-close" @click="deleteUser">删除</el-button>
        </div>
      </div>
      <!---------------------------- 表格 ---------------------------->
    ...
    <!---------------------------- 权限维护弹窗 ---------------------------->
    <auth-drawer ref="authDrawer"></auth-drawer>
    <!---------------------------- 添加用户弹窗 ---------------------------->
  ...
  methods: {
    ...
    deleteRole(row) {
      ...
      fetchUserList(data)
        .then(res => {
          if (res.success) {
            if (res.total === 0) {
              this.$confirm(`是否确认删除『 ${row.NAME} 』角色？`, "提示", {
    ...
    openUserDrawer() {
      if (this.selectRoleId) {
        ...
        userDrawer.roleId = this.selectRoleId;
    ...
    deleteUser() {
      ...
      let names = this.selectedRows.map(i => i.NAME).join("，");
      this.$confirm(
        `是否确认将『 ${names} 』从『 ${this.selectRoleName} 』中删除？`,
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
