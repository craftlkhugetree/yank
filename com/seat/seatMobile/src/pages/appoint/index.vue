        ...
        >
          <div v-for="(v,i) in appointList" :key="i">
            <div class="single_occupy_seat">
              ...
              <h3 v-else>{{v.name}}</h3>
              <p v-if="v.orderunit=='week'">开放时间：{{v.starttime}}～{{v.endtime}}</p>
              <p v-if="v.orderunit=='day'">开放时间：{{v.starttime}}～{{v.endtime}}</p>
              <h4 :class="{'fulltext':v.unusenum==0}">当前剩余：{{v.unusenum}}座</h4>
              <i @click="goAppoint(v)" :class="{'isBlack':isBlack}" v-if="v.unusenum!=0">预约</i>
