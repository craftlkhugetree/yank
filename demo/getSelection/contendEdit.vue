        ...
        if (
          ...
          if (
            getSelection().focusOffset ===
              parentNode.textContent.trim().length &&
            e.keyCode !== 8
          ) {
            // span标签尾部输入 code18是删除 就直接删除
            this.$nextTick(() => {
              let sel = null;
              ...
              if (obj.childNodes.length - 1 <= position) {
                // 在最后 而且最后还没其他node节点 那么就在最后插入一个空格节点
                obj.innerHTML = obj.innerHTML + "&nbsp;";
              }
              let textContent = obj.childNodes[position].textContent;
              if (obj.childNodes[position]) {
                if (obj.childNodes[position].nodeType === 1) {
                  // span节点
