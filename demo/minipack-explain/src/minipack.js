...
// 该函数将只接收一个参数: 一个包含`graph`中每个模块信息的对象, 上面的queue.
function bundle(graph) {
  ...
  // 自执行函数的参数是 {${modules}} ，是将modules字符串用{}包裹，变成了对象，该对象key为0，1，2，3等id，value为数组[fn, obj]。 
  // 先执行require(0); 而localRequire(name)返回的是一个函数require(id)
  // modules[id]解析出的fn和mapping就是id的value值，fn就变成了  function(require, module, exports) {${mod.code}},   这里的require和node.js的require同名。
